...
void idSessionLocal::HandleMainMenuCommands( const char *menuCommand ) {
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		...
		if ( !idStr::Icmp( cmd, "startGame" ) ) {
			...
			cvarSystem->SetCVarString( "si_gameType", "singleplayer" );	// sikk - Added
			cvarSystem->SetCVarString( "g_energySystem", "1" );	// sikk - Added
			if ( icmd < args.Argc() ) {
				StartNewGame( args.Argv( icmd++ ) );
			} else {
#ifndef ID_DEMO_BUILD
				StartNewGame( "game/fb_start" );	// sikk - was mars_city1
#else
...
// ---> sikk - Survival Gamemode
		if ( !idStr::Icmp( cmd, "startSurvival" ) ) {
			const char *map = cvarSystem->GetCVarString( "si_map" );
			cvarSystem->SetCVarString( "si_gameType", "survival" );

			if ( map[0] == '\0' ) {
				cvarSystem->SetCVarString( "si_map", "sv1" );
				StartNewSVGame( "sv1" );
			} else {
				StartNewSVGame( map );
			}
