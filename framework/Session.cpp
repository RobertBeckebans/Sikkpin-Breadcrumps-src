...
// ---> sikk - Survival Gamemode
/*
===============
idSessionLocal::StartNewSVGame
===============
*/
void idSessionLocal::StartNewSVGame( const char *mapName, bool devmap ) {
#ifdef	ID_DEDICATED
	common->Printf( "Dedicated servers cannot start singleplayer games.\n" );
	return;
#else
	...
	mapSpawnData.serverInfo.Clear();
	mapSpawnData.serverInfo = *cvarSystem->MoveCVarsToDict( CVAR_SERVERINFO );
	mapSpawnData.serverInfo.Set( "si_gameType", "survival" );
...
void idSessionLocal::ExecuteMapChange( bool noFadeWipe ) {
	...
	// mute sound
	soundSystem->SetMute( true );	// sikk - Commented to allow background music during level load
	...
	menuSoundWorld->ClearAllSoundEmitters();	// sikk - 
	...
	soundSystem->SetMute( false );	// sikk - we don't need to un-mute
...
void idSessionLocal::RunGameTic() {
	...
	if ( logIndex < MAX_LOGGED_USERCMDS ) {
		...
		if (logIndex % 30 == 0 && statIndex < MAX_LOGGED_STATS) {
			...
			loggedStats[statIndex].heartRate = 0;	// sikk - Removed heartrate in game code - was - ret.heartRate;
			loggedStats[statIndex].stamina = 0;	// sikk - Removed stamnia in game code - was - ret.stamina;
