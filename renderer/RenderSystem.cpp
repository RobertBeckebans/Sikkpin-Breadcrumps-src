...
/*
=============
R_AddDrawSSAOViewCmd

This is the main 3D rendering command.  A single scene may
have multiple views if a mirror, portal, or dynamic texture is present.
=============
*/
void R_AddDrawSSAOViewCmd( viewDef_t *parms ) {
	drawSurfsCommand_t	*cmd;

	cmd = (drawSurfsCommand_t *)R_GetCommandBuffer( sizeof( *cmd ) );
	cmd->commandId = RC_DRAW_SSAOVIEW;
...
=============
*/
void idRenderSystemLocal::SetShaderParm( int parm, float value ) {
	guiModel->SetShaderParm( parm, value );
}
// <--- sikk - pass shader parms to GUI model for post FX
...
================
*/
void idRenderSystemLocal::CaptureRenderToImage( const char *imageName, bool nearest ) {	// sikk - nearest texture filter for framebuffer - Added 'bool nearest' arg
	if ( !glConfig.isInitialized ) {
		return;
	}
	guiModel->EmitFullScreen();
...
// ---> sikk - nearest texture filter for framebuffer
	if ( nearest ) {
		cmd->commandId = RC_COPY_RENDER_NEAREST;
	} else {
// <--- sikk - nearest texture filter for framebuffer
...
void idRenderSystemLocal::CaptureDepthToImage( const char *imageName ) {
	...
	// look up the image before we create the render command, because it
	// may need to sync to create the image
	idImage	*image = globalImages->ImageFromFile( imageName, TF_NEAREST, true, TR_CLAMP_TO_ZERO, TD_HIGH_QUALITY );
