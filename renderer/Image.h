...
public:
	...
	void		CopyStencilbuffer( int x, int y, int width, int height );	// sikk - Copy Stencil Buffer
...
public:
	...
	idImage *			ImageFromFile( const char *name,
							 textureFilter_t filter, bool allowDownSize,
							 textureRepeat_t repeat, textureDepth_t depth, cubeFiles_t cubeMap = CF_2D, bool doGammaCorrection = false );	// sikk - Gamma Correction - Added "bool gammaCorrect = false" arg
	...
	idImage *			mirrorRenderImage;			// sikk - mirror render specific image
	idImage *			portalRenderImage;			// sikk - mirror render specific image
	idImage *			currentDepthImage;			// sikk - copy of the current depth buffer
	idImage *			ssaoImage;					// sikk - ssao buffer
...
byte *R_MipMapWithGamma( const byte *in, int width, int height, bool preserveBorder );	// sikk - Gamma Correction
