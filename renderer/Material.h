...
typedef enum {
	...
	SL_DIFFUSE,
	SL_SPECULAR,
	SL_SHADER	// sikk - Custom Interaction Shader stage
} stageLighting_t;
...
static const int	MAX_VERTEX_PARMS = 8;	// sikk - was 4
static const int	MAX_FRAGMENT_PARMS = 8;	// sikk - Fragment Program Parms
static const int	MAX_INTERACTION_IMAGES = 10;	// sikk - Auxilary textures for interaction shaders
...
const int MAX_ENTITY_SHADER_PARMS	= 32;	//sikk - was 12;
...
public:
	...
	float				GetEditorScale( void ) const { return editorScale; }	// sikk - Default Editor Scale
	...
	bool				NeedsCurrentRender( void ) const { return needCurrentRender; };	// sikk - Material needs _currentRender

// ---> sikk - Custom Interaction Shaders
	idStr				GetInteractionVS_Dir() const	{ return interactionVS_Dir; };
	idStr				GetInteractionVS_Amb() const	{ return interactionVS_Amb; };
	idStr				GetInteractionFS_Dir() const	{ return interactionFS_Dir; };
	idStr				GetInteractionFS_Amb() const	{ return interactionFS_Amb; };

	int					GetInteractionVP_Dir() const	{ return interactionVP_Dir; };
	int					GetInteractionVP_Amb() const	{ return interactionVP_Amb; };
	int					GetInteractionFP_2D() const		{ return interactionFP_2D; };
	int					GetInteractionFP_3D() const		{ return interactionFP_3D; };
	int					GetInteractionFP_Sun() const	{ return interactionFP_Sun; };
	int					GetInteractionFP_Amb() const	{ return interactionFP_Amb; };
// <--- sikk - Custom Interaction Shaders

// ---> sikk - Custom Interaction Shaders: Local Parameters
	int					GetNumInteractionParms() const { return numInteractionParms; };
	int					GetInteractionParm( int i, int j ) const { return interactionParm[ i ][ j ]; };
// <--- sikk - Custom Interaction Shaders: Local Parameters

// ---> sikk - Auxilary textures for interaction shaders
	void				ParseInteractionMap( idLexer &src );
	int					GetNumInteractionImages() const { return numInteractionImages; };
	idImage	*			GetInteractionImage( int num ) const { return interactionImages[ num ]; };
// <--- sikk - Auxilary textures for interaction shaders

	int					GetSpecExp( int i ) const { return specExp[ i ]; };	// sikk - Specular Exponent Scale/Bias
...
private:
	...
	void				ParseFragmentParm( idLexer &src, newShaderStage_t *newStage );	// sikk - Fragment Program Parms
...
private:
	...
	float				editorAlpha;
	float				editorScale;	// sikk - Default Editor Scale

	bool				suppressInSubview;
	bool				portalSky;
	int					refCount;

	bool				needCurrentRender;	// sikk - Material needs _currentRender

// ---> sikk - Custom Interaction Shaders
	idStr				interactionVS_Dir;
	idStr				interactionVS_Amb;
	idStr				interactionFS_Dir;
	idStr				interactionFS_Amb;

	int					interactionVP_Dir;
	int					interactionVP_Amb;
	int					interactionFP_2D;
	int					interactionFP_3D;
	int					interactionFP_Sun;
	int					interactionFP_Amb;
	int					interactionParm[ MAX_FRAGMENT_PARMS ][ 4 ];
	int					numInteractionParms;
// <--- sikk - Custom Interaction Shaders

// ---> sikk - Auxilary textures for interaction shaders
	idImage *			interactionImages[ MAX_INTERACTION_IMAGES ];
	int					numInteractionImages;
// <--- sikk - Auxilary textures for interaction shaders

	int					specExp[ 2 ];	// - sikk - Specular Exponent Scale/Bias
};
