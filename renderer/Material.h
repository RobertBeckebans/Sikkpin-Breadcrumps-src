...
public:
	...
	{
		return editorScale;    // sikk - Default Editor Scale
	}
	...
	bool				NeedsCurrentRender() const
	{
		return needCurrentRender;
	};	// sikk - Material needs _currentRender

// ---> sikk - Custom Interaction Shaders
	idStr				GetInteractionVS_Dir() const
	{
		return interactionVS_Dir;
	};
	idStr				GetInteractionVS_Amb() const
	{
		return interactionVS_Amb;
	};
	idStr				GetInteractionFS_Dir() const
	{
		return interactionFS_Dir;
	};
	idStr				GetInteractionFS_Amb() const
	{
		return interactionFS_Amb;
	};

	int					GetInteractionVP_Dir() const
	{
		return interactionVP_Dir;
	};
	int					GetInteractionVP_Amb() const
	{
		return interactionVP_Amb;
	};
	int					GetInteractionFP_2D() const
	{
		return interactionFP_2D;
	};
	int					GetInteractionFP_3D() const
	{
		return interactionFP_3D;
	};
	int					GetInteractionFP_Sun() const
	{
		return interactionFP_Sun;
	};
	int					GetInteractionFP_Amb() const
	{
		return interactionFP_Amb;
	};
// <--- sikk - Custom Interaction Shaders

// ---> sikk - Custom Interaction Shaders: Local Parameters
	int					GetNumInteractionParms() const
	{
		return numInteractionParms;
	};
	int					GetInteractionParm( int i, int j ) const
	{
		return interactionParm[ i ][ j ];
	};
// <--- sikk - Custom Interaction Shaders: Local Parameters

// ---> sikk - Auxilary textures for interaction shaders
	void				ParseInteractionMap( idLexer& src );
	int					GetNumInteractionImages() const
	{
		return numInteractionImages;
	};
	idImage*				GetInteractionImage( int num ) const
	{
		return interactionImages[ num ];
	};
// <--- sikk - Auxilary textures for interaction shaders

	int					GetSpecExp( int i ) const
	{
		return specExp[ i ];
