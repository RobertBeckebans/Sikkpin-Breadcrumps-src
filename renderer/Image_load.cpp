...
// ---> sikk - Added alpha check since we are using the spec's alpha for gloss
			if ( needAlpha ) {
...
void idImage::SetImageFilterAndRepeat() const {
	...
	case TF_LINEAR:
		...
		break;
	case TF_NEAREST:
		qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST );	// sikk - Changed from GL_NEAREST
...
// ---> sikk - Percentage Downsize test
		if ( globalImages->image_downSizeSpecular.GetInteger() > 1 ) {
			size = max( scaled_width, scaled_height ) / globalImages->image_downSizeSpecularLimit.GetInteger();
		} else {
// <--- sikk - Percentage Downsize test
...
// ---> sikk - Percentage Downsize test
		if ( globalImages->image_downSizeBump.GetInteger() > 1 ) {
			size = max( scaled_width, scaled_height ) / globalImages->image_downSizeBumpLimit.GetInteger();
		} else {
// <--- sikk - Percentage Downsize test
...
// ---> sikk - Percentage Downsize test
		if ( globalImages->image_downSize.GetInteger() > 1 ) {
			size = max( scaled_width, scaled_height ) / globalImages->image_downSizeLimit.GetInteger();
		} else {
// <--- sikk - Percentage Downsize test
...
// ---> sikk - Gamma Correction
		if ( gammaCorrect && r_sRGB.GetBool() ) {
			for ( int i = 0; i < scaled_width * scaled_height * 4; i += 4 ) {
				for ( int j = 0; j < 3; j++ ) {
					float temp = scaledBuffer[ i + j ];
					temp /= 255;
					if ( temp <= 0 ) temp = 0;
					else if ( temp <= 0.04045f ) temp /= 12.92f;
					else temp = pow( ( temp + 0.055f ) / 1.055f, 2.4f );
					scaledBuffer[ i + j ] = idMath::Ftoi( temp * 255 );
				}
			}
		}
// <--- sikk - Gamma Correction
...
// ---> sikk - Gamma Correction
		if ( gammaCorrect && r_sRGB.GetBool() ) {
			scaledBuffer = (byte *)R_StaticAlloc( sizeof( unsigned ) * width * height );
			memcpy( scaledBuffer, pic, width * height * 4 );
			...
			scaledBuffer = R_MipMapWithGamma( scaledBuffer, width, height, preserveBorder );
		} else {
// <--- sikk - Gamma Correction
...
// ---> sikk - Gamma Correction
			if ( gammaCorrect && r_sRGB.GetBool() ) {
				shrunk = R_MipMapWithGamma( scaledBuffer, width, height, preserveBorder );
			} else {
// <--- sikk - Gamma Correction
...
// sikk - removed normal component swapping
...
// ---> sikk - Gamma Correction
		if ( gammaCorrect && r_sRGB.GetBool() ) {
			shrunk = R_MipMapWithGamma( scaledBuffer, scaled_width, scaled_height, preserveBorder );
		} else {
// <--- sikk - Gamma Correction
...
void idImage::Generate3DImage( const byte *pic, int width, int height, int picDepth,
	...
	case TF_LINEAR:
		...
		break;
	case TF_NEAREST:
		qglTexParameterf(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST );	// sikk - Changed from GL_NEAREST
...
void idImage::GenerateCubeImage( const byte *pic[6], int size, 
	...
	case TF_LINEAR:
		...
		break;
	case TF_NEAREST:
		qglTexParameterf(GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST );	// sikk - Changed from GL_NEAREST
...
// ---> sikk - Commented all of the PoT shit. No more NPoT Adjust BS!!! 
...
// - sikk	qglCopyTexImage2D( GL_TEXTURE_2D, 0, GL_RGB8, x, y, imageWidth, imageHeight, 0 );
...
*/
// ---> sikk - nearest texture filter for framebuffer
...
//	void	*stencilReadback;
//	stencilReadback = R_StaticAlloc( glConfig.vidWidth * glConfig.vidHeight * 4 );
//	memset( stencilReadback, 0, glConfig.vidWidth * glConfig.vidHeight * 4 );
//	qglReadPixels( 0, 0, glConfig.vidWidth, glConfig.vidHeight, GL_STENCIL_INDEX, GL_FLOAT, stencilReadback );
//#if 1
//	for ( int i = 0 ; i < glConfig.vidWidth * glConfig.vidHeight ; i++ ) {
//		((byte *)stencilReadback)[i*4] = 
//		((byte *)stencilReadback)[i*4+1] = 
//		((byte *)stencilReadback)[i*4+2] = ((float *)stencilReadback)[i] <= 128 ? 255 : 0;
//		((byte *)stencilReadback)[i*4+3] = 1;
//	}
//#endif
//	GenerateImage( (byte *)stencilReadback, 1024, 512, TF_LINEAR, false, TR_CLAMP, TD_HIGH_QUALITY );
//	R_StaticFree( stencilReadback );
	...
	//	uploadHeight = potHeight;
	//	if ( potWidth == imageWidth && potHeight == imageHeight ) {
		//	qglCopyTexImage2D( GL_TEXTURE_2D, 0, GL_STENCIL_INDEX, x, y, imageWidth, imageHeight, 0 );
	//	} else {
	//		// we need to create a dummy image with power of two dimensions,
	//		// then do a qglCopyTexSubImage2D of the data we want
			qglTexImage2D( GL_TEXTURE_2D, 0, GL_STENCIL_INDEX, imageWidth, imageWidth, 0, GL_STENCIL_INDEX, GL_UNSIGNED_BYTE, NULL );
			qglCopyTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, x, y, imageWidth, imageHeight );
	//	}
	//} else {
	//	// otherwise, just subimage upload it so that drivers can tell we are going to be changing
