...
====================
*/
void idRenderWorldLocal::RenderSceneSSAO( const renderView_t *renderView ) {
#ifndef	ID_DEDICATED
	renderView_t	copy;
	...
	if ( renderView->fov_x <= 0 || renderView->fov_y <= 0 ) {
		common->Error( "idRenderWorld::RenderSceneSSAO: bad FOVs: %f, %f", renderView->fov_x, renderView->fov_y );
	}
...
// ---> sikk - Increased interactionTable size
//		interactionTableWidth = entityDefs.Num() + 100;
//		interactionTableHeight = lightDefs.Num() + 100;
		interactionTableWidth = entityDefs.Num() + 1024;
		interactionTableHeight = lightDefs.Num() + 1024;
// <--- sikk - Increased interactionTable size
