...
typedef enum {
	RC_NOP,
	RC_DRAW_VIEW,
	RC_DRAW_SSAOVIEW,	// sikk - ssao test
	RC_SET_BUFFER,
	RC_COPY_RENDER,
	RC_COPY_RENDER_NEAREST,	// sikk - Added - nearest texture filter for framebuffer
	RC_COPY_DEPTH,			// sikk - Capture Depth Buffer to Image
...
void R_AddDrawSSAOViewCmd( viewDef_t *parms );	// sikk - ssao test
...
const int MAX_MULTITEXTURE_UNITS =	16;//8;	// sikk - Increased Max TUs for auxilary interaction shaders
typedef struct {
...
typedef struct {
	...
	const viewDef_t *	previousviewDef;	// sikk - save previous space info for motion blur

	backEndCounters_t	pc;

	const viewEntity_t *currentSpace;		// for detecting when a matrix must change
	const viewEntity_t		*previousSpace;		// sikk - save previous space info for motion blur
...
// <--- sikk - Removed obsolete render paths
	BE_ARB2,
	BE_GLSL,	// sikk - raynorpat's GLSL Support
	BE_BAD
} backEndName_t;
...
public:
	...
	virtual void			SetShaderParm( int parm, float value );	// sikk - pass shader parms to GUI model for post FX
	...
	virtual void			CaptureRenderToImage( const char *imageName, bool nearest = false );	// sikk - nearest texture filter for framebuffer -Added 'bool nearest' arg
	virtual void			CaptureRenderToFile( const char *fileName, bool fixAlpha );

	virtual void			CaptureDepthToImage( const char *imageName );	// sikk - Capture Depth Buffer to Image
...
// ---> sikk - These are useful for shaders
	float motionMatrix[16];	// sikk - Motion Matrix
	float prevMVPMatrix[16];	// sikk - Previous Frame's Projection Matrix
	float projMatrix[16];	// sikk - Projection Matrix
	float fov[4];			// sikk - FOV 
	float focalLen[4];		// sikk - Focal Length
// <--- sikk - These are useful for shaders
};
...
// ---> sikk - New CVAR's
extern idCVar r_useParallaxMapping;		// override all materials
extern idCVar r_shaderQuality;			// shader quality

extern idCVar r_fullbright;				// full bright
extern idCVar r_sRGB;					// Gamma Correction
extern idCVar r_showStencil;			// display the contents of the stencil buffer

// ---> sikk - Light type skipping
extern idCVar r_skipAmbientLights;		// skip all ambient lights
extern idCVar r_skipPointLights;		// skip all point lights
...
void R_RenderViewSSAO( viewDef_t *parms );	// sikk - ssao test
...
void RB_DrawSSAOView( const void *data );	// sikk - ssao test
...
// ---> sikk - Removed unecessary shaders/ Added custom shaders
	...
	VPROG_INTERACTIONDIR,
	FPROG_INTERACTION2D,
	FPROG_INTERACTION3D,
	FPROG_INTERACTIONSUN,
	VPROG_INTERACTIONAMB,
	FPROG_INTERACTIONAMB,

	VPROG_ENVIRONMENT,
	FPROG_ENVIRONMENT,
	VPROG_ENVIRONMENTBUMP,
	FPROG_ENVIRONMENTBUMP,
	...
	PROG_USER
// <--- sikk - Removed unecessary shaders
} program_t;
...
extern shaderProgram_t	interactionDirShader;
extern shaderProgram_t	interactionAmbShader;
...
// <--- sikk - added
	...
	PP_LIGHT_PROJECT_S,
	PP_LIGHT_PROJECT_T,
	PP_LIGHT_FALLOFF_S,	// sikk - switched order for these two
	PP_LIGHT_PROJECT_Q,	// sikk - switched order for these two
	PP_BUMP_MATRIX_S,
	PP_BUMP_MATRIX_T,
	PP_DIFFUSE_MATRIX_S,
	PP_DIFFUSE_MATRIX_T,
	...
	PP_MODEL_MATRIX_Y,
	PP_MODEL_MATRIX_Z,
	PP_COLOR_MODULATE_ADD,
	PP_AMBIENT_COLOR_X_NEG,
	PP_AMBIENT_COLOR_X_POS,
	PP_AMBIENT_COLOR_Y_NEG,
	PP_AMBIENT_COLOR_Y_POS,
	PP_AMBIENT_COLOR_Z_NEG,
	PP_AMBIENT_COLOR_Z_POS
} programParameter_t;
