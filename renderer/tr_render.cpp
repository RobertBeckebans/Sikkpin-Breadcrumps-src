...
void RB_DrawElementsWithCounters( const srfTriangles_t *tri ) {
	...
	if ( tri->indexCache && r_useIndexBuffers.GetBool() ) {
		...
		//	vertexCache.UnbindIndex();
			vertexCache.UnbindIndex( GL_ELEMENT_ARRAY_BUFFER );	// sikk - Mh & reckless VBO
		}
		qglDrawElements( GL_TRIANGLES, 
						r_singleTriangle.GetBool() ? 3 : tri->numIndexes,
						GL_INDEX_TYPE,
...
// ---> sikk - Removed color clamping (this is color in the material stage)
...
static void RB_SubmittInteraction( drawInteraction_t *din, void (*DrawInteraction)(const drawInteraction_t *) ) {
	...
	if ( !din->specularImage || r_skipSpecular.GetBool() /*|| din->ambientLight*/ ) {	// sikk - Specular in ambient pass
...
// sikk -
	...
	//if ( ( ( din->diffuseColor[0] > 0 || din->diffuseColor[1] > 0 || din->diffuseColor[2] > 0 ) &&
	//	 din->diffuseImage != globalImages->blackImage ) ||
	//	 ( ( din->specularColor[0] > 0 || din->specularColor[1] > 0 || din->specularColor[2] > 0 ) &&
	//	 din->specularImage != globalImages->blackImage ) ) {
	if ( ( din->diffuseColor[0] > 0 || din->diffuseColor[1] > 0 || din->diffuseColor[2] > 0 ) &&
		 ( din->diffuseImage != globalImages->blackImage || din->specularImage != globalImages->blackImage ) ) {
		DrawInteraction( din );
	}
}
...
void RB_CreateSingleDrawInteractions( const drawSurf_t *surf, void (*DrawInteraction)(const drawInteraction_t *) ) {
	...
	inter.ambientLight = vLight->lightDef->parms.ambient;	// sikk - Ambient Light Flag - was lightShader->IsAmbientLight();
...
// ---> sikk - diffuse light color gamma correction
		...
		for ( int i = 0; i < 3; i++ ) {
			if ( lightColor[ i ] <= 0.0f ) lightColor[ i ] = 0;
			else if ( lightColor[ i ] <= 0.04045f ) lightColor[ i ] /= 12.92f;
			else lightColor[ i ] = pow( ( lightColor[ i ] + 0.055f ) / 1.055f, 2.4f );
			lightColor[ i ] *= backEnd.lightScale * lightColor[3];
		}
// <--- sikk - diffuse light color gamma correction
...
// ---> sikk - Use single light color value for both diffuse & specular
					R_SetDrawInteraction( surfaceStage, surfaceRegs, &inter.specularImage,
											inter.specularMatrix, inter.diffuseColor.ToFloatPtr() );
					inter.diffuseColor[0] *= lightColor[0];
					inter.diffuseColor[1] *= lightColor[1];
					inter.diffuseColor[2] *= lightColor[2];
					inter.diffuseColor[3] *= lightColor[3];
...
// ---> sikk - Custom Interaction Shader stage
				...
				//	R_SetDrawInteraction( surfaceStage, surfaceRegs, &inter.specularImage, inter.specularMatrix, NULL );
				//	inter.diffuseColor[0] *= lightColor[0];
				//	inter.diffuseColor[1] *= lightColor[1];
				//	inter.diffuseColor[2] *= lightColor[2];
				//	inter.diffuseColor[3] *= lightColor[3];
