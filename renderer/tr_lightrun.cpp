...
void R_DeriveLightData( idRenderLightLocal *light ) {
	...
	if ( !light->lightShader ) {
		if ( light->parms.pointLight ) {
			if ( light->parms.parallel ) {	// sikk - added
				light->lightShader = declManager->FindMaterial( "lights/defaultParallelLight" );
			} else {
	...
	if ( !light->falloffImage ) {
		...
		if ( light->parms.pointLight ) {
			if ( light->parms.parallel ) {	// sikk - added
				defaultShader = declManager->FindMaterial( "lights/defaultParallelLight" );
			} else {
