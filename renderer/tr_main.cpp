...
// ---> sikk - Limited Far z
	if ( r_useInfiniteFarZ.GetBool() ) {
		tr.viewDef->projectionMatrix[10] = -0.999f;
		tr.viewDef->projectionMatrix[14] = -2.0f * zNear;
	} else {
		float temp = 1.0f / ( 131072.0f - zNear );  
		tr.viewDef->projectionMatrix[10] = -( ( 131072.0f + zNear ) * temp );
		tr.viewDef->projectionMatrix[14] = -( ( 2.0f * 131072.0f * zNear ) * temp );
	}
// <--- sikk - Limited Far z
...
// ---> sikk - Useful things for shaders that aren't saved or overridden in the viewdef for post process effects
	// sikk - Projection Matrix
	float temp[16];
	float temp2[16];
	float temp3[16];
	idMat4 mat, mat2;
	
	R_TransposeGLMatrix( tr.viewDef->projectionMatrix, temp2 );
	R_TransposeGLMatrix( tr.viewDef->worldSpace.modelViewMatrix, temp3 );
	myGlMultMatrix( temp2, temp3, temp );
	for ( int i = 0; i < 4; i++ ) {
		mat[i][0] = temp[i];
		mat[i][1] = temp[i + 4];
		mat[i][2] = temp[i + 8];
	...
	myGlMultMatrix(	tr.prevMVPMatrix, temp2, tr.motionMatrix );

	for ( int i = 0; i < 16; i++ ) {
		tr.prevMVPMatrix[i] = temp[i];
		tr.projMatrix[i] = tr.viewDef->projectionMatrix[i];
	}

	int w = tr.viewDef->viewport.x2 - tr.viewDef->viewport.x1 + 1;
	int h = tr.viewDef->viewport.y2 - tr.viewDef->viewport.y1 + 1;
	float aspect = (float)w / h;
	float fovX = DEG2RAD( tr.viewDef->renderView.fov_x * 0.5f );
	float fovY = DEG2RAD( tr.viewDef->renderView.fov_y * 0.5f );
	//float cosFovY = idMath::Cos( fovY );
	//float sinFovY = idMath::Sin( fovY );
	float invFocalLenY = 1.0f / tr.viewDef->projectionMatrix[5];//sinFovY / cosFovY;
	float invFocalLenX = 1.0f / tr.viewDef->projectionMatrix[0];//invFocalLenY * aspect;
	tr.fov[0] = fovX;
	tr.fov[1] = fovY;
	tr.fov[2] = w;//1.0f / aspect;
	tr.fov[3] = h;//1.0f;
	tr.focalLen[0] = 1.0f / invFocalLenX;
	tr.focalLen[1] = 1.0f / invFocalLenY;
	tr.focalLen[2] = invFocalLenX;
	tr.focalLen[3] = invFocalLenY;
}
...
static void R_SortDrawSurfs( void ) {
#if 0
	uint * indices = (uint *) _alloca16( tr.viewDef->numDrawSurfs * sizeof( indices[0] ) );

	// sort the draw surfs based on:
	// 1. sort value (largest first)
	// 2. depth (smallest first)
	// 3. index (largest first)
	assert( tr.viewDef->numDrawSurfs <= 0xFFFF );
	for ( int i = 0; i < tr.viewDef->numDrawSurfs; i++ ) {
		float sort = SS_POST_PROCESS - tr.viewDef->drawSurfs[i]->sort;
		assert( sort >= 0.0f );

		uint dist = 0;
		if ( tr.viewDef->drawSurfs[i]->geo != NULL ) {
			float min = 0.0f;
			float max = 1.0f;
			// sikk - need to replicate this function
			idRenderMatrix::DepthBoundsForBounds( min, max, tr.viewDef->drawSurfs[i]->space->mvp, tr.viewDef->drawSurfs[i]->geo->bounds );
			dist = idMath::Ftoi( min * 0xFFFF );
		}
		
		indices[i] = ( ( tr.viewDef->numDrawSurfs - i ) & 0xFFFF ) | ( dist << 16 ) | ( (uint) ( *(uint *)&sort ) << 32 );
	}
	...
	register int st_lo = 0;
	register int st_hi = tr.viewDef->numDrawSurfs - 1;
	register int level = 0;

	for ( ; ; ) {
		register int i = st_lo;
		register int j = st_hi;
		if ( j - i >= 4 && level < MAX_LEVELS - 1 ) {
			register uint pivot = indices[( i + j ) / 2];
			do {
				while ( indices[i] > pivot ) i++;
				while ( indices[j] < pivot ) j--;
				if ( i > j ) break;
				uint h = indices[i]; indices[i] = indices[j]; indices[j] = h;
			} while ( ++i <= --j );
			...
			for( ; i < j; j-- ) {
				...
				uint h = indices[m]; indices[m] = indices[j]; indices[j] = h;
			}
			if ( --level < 0 ) {
				break;
			}
	...
	for ( int i = 0; i < tr.viewDef->numDrawSurfs; i++ ) {
		newDrawSurfs[i] = tr.viewDef->drawSurfs[ tr.viewDef->numDrawSurfs - ( indices[i] & 0xFFFF ) ];
	}
	memcpy( tr.viewDef->drawSurfs, newDrawSurfs, tr.viewDef->numDrawSurfsnumDrawSurfs * sizeof( tr.viewDef->drawSurfs[0] ) );
...
================
*/
void R_RenderViewSSAO( viewDef_t *parms ) {
	viewDef_t		*oldView;
