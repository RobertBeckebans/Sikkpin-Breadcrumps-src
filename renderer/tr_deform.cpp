...
static void R_TubeDeform( drawSurf_t* surf )
{
	...
	if( tri->numVerts & 3 )
	{
		common->Error( "R_TubeDeform: shader had odd vertex count" );	// sikk - was "R_AutospriteDeform"
	}
	if( tri->numIndexes != ( tri->numVerts >> 2 ) * 6 )
	{
		common->Error( "R_TubeDeform: autosprite had odd index count" );	// sikk - was "R_AutospriteDeform"
	}
