...
void idMaterial::CommonInit() {
	...
	editorAlpha = 1.0;
	editorScale = 1.0;	// sikk - Default Editor Scale
	spectrum = 0;
	polygonOffset = 0;
	...
	needCurrentRender = false;		// sikk - Material needs _currentRender

// ---> sikk - Custom Interaction Shaders
	// this sets the default shaders (see program_t enum def in tr_local.h)
	interactionVP_Dir	= 2;
	interactionFP_2D	= 3;
	interactionFP_3D	= 4;
	interactionFP_Sun	= 5;
...
// ---> sikk - Auxilary textures for interaction shaders
	numInteractionImages = 0;
	for ( int i = 0; i < MAX_INTERACTION_IMAGES; i++ ) {
		interactionImages[ i ] = NULL;
	}
// <--- sikk - Auxilary textures for interaction shaders
...
// ---> sikk - Increased Local Registers
	if ( !token.Icmp( "parm12" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM12;
	}
	if ( !token.Icmp( "parm13" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM13;
	}
	if ( !token.Icmp( "parm14" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM14;
	}
	if ( !token.Icmp( "parm15" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM15;
	}
	if ( !token.Icmp( "parm16" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM16;
	}
	if ( !token.Icmp( "parm17" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM17;
	}
	if ( !token.Icmp( "parm18" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM18;
	}
	if ( !token.Icmp( "parm19" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM19;
	}
	if ( !token.Icmp( "parm20" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM20;
	}
	if ( !token.Icmp( "parm21" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM21;
	}
	if ( !token.Icmp( "parm22" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM22;
	}
	if ( !token.Icmp( "parm23" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM23;
	}
	if ( !token.Icmp( "parm24" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM24;
	}
	if ( !token.Icmp( "parm25" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM25;
	}
	if ( !token.Icmp( "parm26" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM26;
	}
	if ( !token.Icmp( "parm27" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM27;
	}
	if ( !token.Icmp( "parm28" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM28;
	}
	if ( !token.Icmp( "parm29" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM29;
	}
	if ( !token.Icmp( "parm30" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM30;
	}
	if ( !token.Icmp( "parm31" ) ) {
		pd->registersAreConstant = false;
		return EXP_REG_PARM31;
	}
// <--- sikk - Increased Registers
...
// ---> sikk - Custom Interaction Shader stage
...
void idMaterial::ParseFragmentMap( idLexer &src, newShaderStage_t *newStage ) {
	...
	cubeFiles_t			cubeMap;
	bool				allowPicmip;
	bool				gammaCorrect;	// sikk - Gamma Correction
	idToken				token;
	...
	td = TD_DEFAULT;
	allowPicmip = true;
	gammaCorrect = false;	// sikk - Gamma Correction
	cubeMap = CF_2D;
	...
	// sikk - Commented - this isn't true anymore
...
// ---> sikk - Gamma Correction
		if ( !token.Icmp( "gammaCorrect" ) ) {
			gammaCorrect = true;
			continue;
		}
// <--- sikk - Gamma Correction
	...
	newStage->fragmentProgramImages[unit] = 
		globalImages->ImageFromFile( str, tf, allowPicmip, trp, td, cubeMap, gammaCorrect );	// sikk - Gamma Correction
...
// - sikk - Removed unecessary texgen/shader 
...
// ---> sikk - RGBA Vertex Color Support
		if ( !token.Icmp( "vertexColorBase" ) ) {
			ss->vertexColor = SVC_BASE;
			continue;
		}
		if ( !token.Icmp( "vertexColorRed" ) ) {
			ss->vertexColor = SVC_RED;
			continue;
		}
		if ( !token.Icmp( "vertexColorGreen" ) ) {
			ss->vertexColor = SVC_GREEN;
			continue;
		}
		if ( !token.Icmp( "vertexColorBlue" ) ) {
			ss->vertexColor = SVC_BLUE;
			continue;
		}
		if ( !token.Icmp( "vertexColorAlpha" ) ) {
			ss->vertexColor = SVC_ALPHA;
			continue;
		}
// <--- sikk - RGBA Vertex Color Support
...
// <--- sikk - Fragment Program Parms
	...
	if ( td == TD_DEFAULT ) {
		...
		case SL_DIFFUSE:
			td = TD_DIFFUSE;
			gammaCorrect = true;	// sikk - Gamma Correction
			break;
		case SL_SPECULAR:
			td = TD_SPECULAR;
			break;
	...
	if ( imageName[0] ) {
		ts->image = globalImages->ImageFromFile( imageName, tf, allowPicmip, trp, td, cubeMap, gammaCorrect );	// sikk - Gamma Correction
		if ( !ts->image ) {
...
// ---> sikk - Added - explicit editor translucency
		else if ( !token.Icmp( "editorAlpha") ) {
			if ( !src.ReadTokenOnLine( &token ) ) {
				editorAlpha = 0.5f;
				continue;
			}
			editorAlpha = token.GetFloatValue();
			continue;
		}
// <--- sikk - Added - explicit editor translucency
// ---> sikk - Added - Default Editor Scale
		else if ( !token.Icmp( "editorScale") ) {
			if ( !src.ReadTokenOnLine( &token ) ) {
				editorScale = 1.0f;
				continue;
			}
			editorScale = token.GetFloatValue();
			continue;
		}
// <--- sikk - Added - Default Editor Scale
		// description
...
// ---> sikk - Custom Interaction Shaders
		// custom interaction
			...
			if ( src.ReadTokenOnLine( &token ) ) {
				interactionVP_Dir	= R_FindARBProgram( GL_VERTEX_PROGRAM_ARB, idStr( token + "_3D.vfp" ).c_str() );
				interactionVP_Amb	= R_FindARBProgram( GL_VERTEX_PROGRAM_ARB, idStr( token + "_Amb.vfp" ).c_str() );

				interactionFP_2D	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_2D.vfp" ).c_str() );
				interactionFP_3D	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_3D.vfp" ).c_str() );
				interactionFP_Sun	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_Sun.vfp" ).c_str() );
				interactionFP_Amb	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_Amb.vfp" ).c_str() );
			}
			continue;
		}
		else if ( !token.Icmp( "interactionShaderVP" ) ) {
			if ( src.ReadTokenOnLine( &token ) ) {
				interactionVS_Dir	= idStr( token + "_Dir.vs" );
				interactionVS_Amb	= idStr( token + "_Amb.vs" );

				interactionVP_Dir	= R_FindARBProgram( GL_VERTEX_PROGRAM_ARB, idStr( token + "_Dir.vp" ).c_str() );
				interactionVP_Amb	= R_FindARBProgram( GL_VERTEX_PROGRAM_ARB, idStr( token + "_Amb.vp" ).c_str() );
			}
			continue;
		}
		else if ( !token.Icmp( "interactionShaderFP" ) ) {
			if ( src.ReadTokenOnLine( &token ) ) {
				interactionFS_Dir	= idStr( token + "_Dir.fs" );
				interactionFS_Amb	= idStr( token + "_Amb.fs" );

				interactionFP_2D	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_2D.fp" ).c_str() );
				interactionFP_3D	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_3D.fp" ).c_str() );
				interactionFP_Sun	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_Sun.fp" ).c_str() );
				interactionFP_Amb	= R_FindARBProgram( GL_FRAGMENT_PROGRAM_ARB, idStr( token + "_Amb.fp" ).c_str() );
			}
			continue;
		}

		// interaction parms
		else if ( !token.Icmp( "interactionParm" ) ) {
			src.ReadTokenOnLine( &token );
			int	parm = token.GetIntValue();
			if ( !token.IsNumeric() || parm < 0 || parm >= MAX_FRAGMENT_PARMS ) {
				common->Warning( "bad interactionParm number\n" );
				continue;
			}
			if ( parm >= numInteractionParms ) {
				numInteractionParms = parm + 1;
			}
			interactionParm[ parm ][ 0 ] = ParseExpression( src );
			src.ReadTokenOnLine( &token );
			if ( !token[ 0 ] || token.Icmp( "," ) ) {
				interactionParm[ parm ][ 1 ] = interactionParm[ parm ][ 2 ] = interactionParm[ parm ][ 3 ] = interactionParm[ parm ][ 0 ];
				continue;
			}
			interactionParm[ parm ][ 1 ] = ParseExpression( src );
			src.ReadTokenOnLine( &token );
			if ( !token[ 0 ] || token.Icmp( "," ) ) {
				interactionParm[ parm ][ 2 ] = GetExpressionConstant( 0 );
				interactionParm[ parm ][ 3 ] = GetExpressionConstant( 1 );
				continue;
			}
			interactionParm[ parm ][ 2 ] = ParseExpression( src );
			src.ReadTokenOnLine( &token );
			if ( !token[ 0 ] || token.Icmp( "," ) ) {
				interactionParm[ parm ][ 3 ] = GetExpressionConstant( 1 );
				continue;
			}
			interactionParm[ parm ][ 3 ] = ParseExpression( src );
			continue;
		}
// <--- sikk - Custom Interaction Shaders

// ---> sikk - Auxilary textures for interaction shaders
		else if ( !token.Icmp( "interactionMap" ) ) {
			ParseInteractionMap( src );
			continue;
		}
// <--- sikk - Auxilary textures for interaction shaders

// ---> sikk - Specular Exponent Scale/Bias
		// specular exponent scale/bias
		else if ( !token.Icmp( "specExp" ) ) {
			src.ReadTokenOnLine( &token );
			if ( !token.IsNumeric() ) {
				common->Warning( "bad specExp number\n" );
				continue;
			}

			specExp[ 0 ] = token.GetFloatValue();
			src.ReadTokenOnLine( &token );
			if ( !token[ 0 ] || token.Icmp( "," ) ) {
				specExp[ 1 ] = 0;
				continue;
...
// ---> sikk - Added contents checks so special brushes draw translucent in the editor
//	if ( /*coverage == MC_TRANSLUCENT ||*/ contentFlags & CONTENTS_AREAPORTAL || 
//		contentFlags & CONTENTS_TRIGGER || contentFlags & CONTENTS_FLASHLIGHT_TRIGGER || 
//		contentFlags & CONTENTS_AAS_SOLID || contentFlags & CONTENTS_AAS_OBSTACLE || 
//		contentFlags & CONTENTS_PLAYERCLIP || contentFlags & CONTENTS_MONSTERCLIP || 
//		contentFlags & CONTENTS_MOVEABLECLIP || contentFlags & CONTENTS_IKCLIP ) {
//// <--- sikk - Added contents check so special brushes draw translucent in the editor
//		editorAlpha = 0.5;
//	} else {
//		editorAlpha = 1.0;
//	}
	...
	for ( i = 0 ; i < numStages ; i++ ) {
		...
		if ( pStage->texture.image == globalImages->currentRenderImage ) {
			// sikk - Removed automatic sort order and automatic translucency
			//if ( sort != SS_PORTAL_SKY ) {
			//	sort = SS_POST_PROCESS;
			//	coverage = MC_TRANSLUCENT;
			//}
			needCurrentRender = true;		// sikk - Material needs _currentRender
			break;
		}
		if ( pStage->newStage ) {
	...
	for ( i = 0 ; i < numStages ; i++ ) {
		shaderStage_t	*pStage = &pd->parseStages[i];
		if ( sort == SS_POST_PROCESS || contentFlags & CONTENTS_WATER  ) {	// sikk - Make liquids draw to the depth buffer so underwater fade works
...
// ---> sikk - Increased Local Registers
	...
	registers[EXP_REG_PARM19] = shaderParms[19];
	registers[EXP_REG_PARM20] = shaderParms[20];
	registers[EXP_REG_PARM21] = shaderParms[21];
	registers[EXP_REG_PARM22] = shaderParms[22];
	registers[EXP_REG_PARM23] = shaderParms[23];
	registers[EXP_REG_PARM24] = shaderParms[24];
	registers[EXP_REG_PARM25] = shaderParms[25];
	registers[EXP_REG_PARM26] = shaderParms[26];
	registers[EXP_REG_PARM27] = shaderParms[27];
	registers[EXP_REG_PARM28] = shaderParms[28];
	registers[EXP_REG_PARM29] = shaderParms[29];
	registers[EXP_REG_PARM30] = shaderParms[30];
	registers[EXP_REG_PARM31] = shaderParms[31];
// <--- sikk - Increased Local Registers
...
/*
================
idMaterial::ParseInteractionMap
================
*/
void idMaterial::ParseInteractionMap( idLexer &src ) {
	const char			*str;
	textureFilter_t		tf;
	textureRepeat_t		trp;
	textureDepth_t		td;
	...
	src.ReadTokenOnLine( &token );
	int	unit = token.GetIntValue();
	if ( !token.IsNumeric() || unit < 0 || unit >= MAX_INTERACTION_IMAGES ) {
		common->Warning( "bad interactionMap number\n" );
		SetMaterialFlag( MF_DEFAULTED );
		return;
	}

	if ( unit >= numInteractionImages ) {
		numInteractionImages = unit + 1;
	}
...
// <--- sikk - Gamma Correction
	...
	interactionImages[ unit ] = globalImages->ImageFromFile( str, tf, allowPicmip, trp, td, cubeMap, gammaCorrect );	// sikk - Gamma Correction
	if ( !interactionImages[ unit ] ) {
		interactionImages[ unit ] = globalImages->defaultImage;
	}
}
// <--- sikk - Auxilary textures for interaction shaders
