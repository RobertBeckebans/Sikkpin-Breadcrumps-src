...
static srfTriangles_t *R_CreateLightTris( const idRenderEntityLocal *ent, 
	...
	if ( r_lightAllBackFaces.GetBool() || light->lightShader->LightEffectsBackSides()
			|| shader->ReceivesLightingOnBackSides()
				|| ent->parms.noSelfShadow || ent->parms.noShadow || !r_shadows.GetBool()  ) {	// sikk - Added '!r_shadows.GetBool()"
...
ID_INLINE bool idInteraction::HasShadows( void ) const {
	return ( !lightDef->parms.noShadows && !entityDef->parms.noShadow && lightDef->lightShader->LightCastsShadows() && !lightDef->parms.ambient );	// sikk - Ambient Light Flag
}
