...
idCVar idSoundSystemLocal::s_minVolume2( "s_minVolume2", "0.0", CVAR_SOUND | CVAR_FLOAT, "" );	// ---> sikk - Better stereo spatialization - default was 0.25
...
idCVar idSoundSystemLocal::s_maxHardwareVoices( "s_maxHardwareVoices", "48", CVAR_SOUND | CVAR_ARCHIVE | CVAR_INTEGER, "maximun number of simutaneous sounds" );	// sikk - Max Sound Channels
	...
	if( idSoundSystemLocal::s_useOpenAL.GetBool() || idSoundSystemLocal::s_useEAXReverb.GetBool() )
	{
		...
		else
		{
			...
			else
			{
				...
				common->Printf( "OpenAL: EAX-RAM extension not found\n" );
			}
...
//	idStr mapname( mapstring );
	idStr mapname( "default" );	// sikk - Use a global efx file that has all default eax environments
...
void idSoundSystemLocal::DoEnviroSuit( float* samples, int numSamples, int numSpeakers )
{
	...
	if( !fxList.Num() )
	{
		for( int i = 0; i < 6; i++ )
		{
			...
			fx = new SoundFX_Comb();
			fx->SetChannel( i );
			fx->SetParameter( 1000 );
			fxList.Append( fx );
			fx = new SoundFX_Comb();
			fx->SetChannel( i );
			fx->SetParameter( 2000 );
			fxList.Append( fx );
			fx = new SoundFX_Comb();
			fx->SetChannel( i );
			fx->SetParameter( 4000 );
			fxList.Append( fx );
		}
	}
