...
void idSoundEmitterLocal::Clear()
{
	...
	prevDistance = 0.0f;	// sikk - Doppler Hack!
	memset( &parms, 0, sizeof( parms ) );
}
...
// ---> sikk - Added - Cuurent amplitude for specifc channel
/*
===================
idSoundEmitterLocal::CurrentAmplitude
...
float idSoundEmitterLocal::CurrentAmplitude( const s_channelType channel )
{
	...
	// calculate a new value
	ampTime = localTime;
	amplitude = soundWorld->FindAmplitude( this, localTime, NULL, channel, false );
...
===================
*/
void idSlowChannel::GenerateSlowChannel( FracTime& playPos, int sampleCount44k, float* finalBuffer, float speed )  	// sikk - Doppler Hack! - added "float speed" arg
{
	idSoundWorldLocal* sw = static_cast<idSoundWorldLocal*>( soundSystemLocal.GetPlayingSoundWorld() );
	float in[8192 + 3], out[8192 + 3], *src, *spline, slowmoSpeed;
	...
	if( sw )
	{
		...
		slowmoSpeed = speed;	// sikk - Doppler Hack!
	}
	else
	{
		slowmoSpeed = 1;
...
===================
*/
void idSlowChannel::GatherChannelSamples( int sampleOffset44k, int sampleCount44k, float* dest, float speed )  	// sikk - Doppler Hack! - added "float speed" arg
{
	int state = 0;
	...
	GenerateSlowChannel( newPosition, sampleCount44k, dest, speed );	// sikk - Doppler Hack! - added "float speed" arg
