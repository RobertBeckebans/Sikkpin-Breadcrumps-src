...
class idSlowChannel
{
	...
	// functions
	void					GenerateSlowChannel( FracTime& playPos, int sampleCount44k, float* finalBuffer, float speed = 1.0f );	// sikk - Doppler Hack! - added "float speed" arg
...
public:
	...
	void					GatherChannelSamples( int sampleOffset44k, int sampleCount44k, float* dest, float speed = 1.0f );	// sikk - Doppler Hack! - added "float speed" arg
...
public:
	...
	virtual	float		CurrentAmplitude( const s_channelType channel );	// sikk - Added - Cuurent amplitude for specifc channel
	...
	float				prevDistance;				// sikk - Doppler Hack! - holds previous distance
...
public:
	...
	static idCVar			s_maxHardwareVoices;	// sikk - Max Sound Channels
};
