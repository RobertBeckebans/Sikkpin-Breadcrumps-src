...
const idMaterial* CNewTexWnd::NextPos()
{
	...
	int width = mat->GetEditorImage()->uploadWidth * ( ( float )g_PrefsDlg.m_nTextureScale / 100 ) * mat->GetEditorScale();	// sikk - Default Editor Scale;
	int height = mat->GetEditorImage()->uploadHeight * ( ( float )g_PrefsDlg.m_nTextureScale / 100 ) * mat->GetEditorScale();	// sikk - Default Editor Scale;
...
// <--- sikk - Texture Win Free Zoom
	...
	//
	UpdateSurfaceDialog();
	//UpdatePatchInspector();	// sikk - Merged Patch Inspector into Surface Inspector
}
...
void CNewTexWnd::OnMouseMove( UINT nFlags, CPoint point )
{
	...
	if( Sys_KeyDown( VK_RBUTTON ) )
	{
		...
		{
// ---> sikk - Texture Win Free Zoom
			if( Sys_KeyDown( VK_MENU ) || Sys_KeyDown( VK_LBUTTON ) )
			{
				long* px = &point.y;
				long* px2 = &cursor.y;

				g_PrefsDlg.m_nTextureScale += ( point.y - cursor.y ) * scale;
				if( g_PrefsDlg.m_nTextureScale < 10 )
				{
					g_PrefsDlg.m_nTextureScale = 10;
				}
				if( g_PrefsDlg.m_nTextureScale > 400 )
				{
					g_PrefsDlg.m_nTextureScale = 400;
				}
...
void Texture_SetMode( int iMenu )
{
	...
	switch( iMenu )
	{
		case ID_VIEW_NEAREST:
			iMode = GL_NEAREST;
			cvarSystem->SetCVarString( "image_filter", "GL_NEAREST" );	// sikk - Added
			break;
		case ID_VIEW_NEARESTMIPMAP:
			iMode = GL_NEAREST_MIPMAP_NEAREST;
			cvarSystem->SetCVarString( "image_filter", "GL_NEAREST_MIPMAP_NEAREST" );	// sikk - Added
			break;
		case ID_VIEW_LINEAR:
			iMode = GL_NEAREST_MIPMAP_LINEAR;
			cvarSystem->SetCVarString( "image_filter", "GL_NEAREST_MIPMAP_LINEAR" );	// sikk - Added
			break;
		case ID_VIEW_BILINEAR:
			iMode = GL_LINEAR;
			cvarSystem->SetCVarString( "image_filter", "GL_LINEAR" );	// sikk - Added
			break;
		case ID_VIEW_BILINEARMIPMAP:
			iMode = GL_LINEAR_MIPMAP_NEAREST;
			cvarSystem->SetCVarString( "image_filter", "GL_LINEAR_MIPMAP_NEAREST" );	// sikk - Added
			break;
		case ID_VIEW_TRILINEAR:
			iMode = GL_LINEAR_MIPMAP_LINEAR;
			cvarSystem->SetCVarString( "image_filter", "GL_LINEAR_MIPMAP_LINEAR" );	// sikk - Added
			break;
...
{
// ---> sikk - Texture Win Free Zoom
	if( Sys_KeyDown( VK_CONTROL ) )
	{
		( zDelta >= 0 ) ? g_PrefsDlg.m_nTextureScale += 10 : g_PrefsDlg.m_nTextureScale -= 10;
		if( g_PrefsDlg.m_nTextureScale < 10 )
		{
			g_PrefsDlg.m_nTextureScale = 10;
		}
		else if( g_PrefsDlg.m_nTextureScale > 400 )
		{
			g_PrefsDlg.m_nTextureScale = 400;
		}
		InvalidateRect( NULL, false );
