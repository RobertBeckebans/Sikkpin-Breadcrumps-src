...
// ---> sikk - Snap Brushes on map import
				for( int n = 0; n < w.GetNumPoints(); n++ )
				{
					w[n].x = floor( w[n].x + 0.5f );
					w[n].y = floor( w[n].y + 0.5f );
					w[n].z = floor( w[n].z + 0.5f );
				}
// <--- sikk - Snap Brushes on map import
...
// ---> sikk - Default Editor Scale
	...
	s1 = ( qtex1 ? static_cast<float>( qtex1->GetEditorImage()->uploadWidth ) * qtex1->GetEditorScale() : 1.0f ) / ( qtex2 ? static_cast<float>( qtex2->GetEditorImage()->uploadWidth ) * qtex2->GetEditorScale() : 1.0f );
	s2 = ( qtex1 ? static_cast<float>( qtex1->GetEditorImage()->uploadHeight ) * qtex1->GetEditorScale() : 1.0f ) / ( qtex2 ? static_cast<float>( qtex2->GetEditorImage()->uploadHeight ) * qtex2->GetEditorScale() : 1.0f );
// <--- sikk - Default Editor Scale
	s1 *= sScale;
	s2 *= tScale;
...
void Face_FitTexture_BrushPrimit( face_t* f, idVec3 mins, idVec3 maxs, float height, float width )
{
	...
	// copy back
// ---> sikk - Added check for width/height != 0 so you can fit just one axis
	if( width != 0 )
	{
