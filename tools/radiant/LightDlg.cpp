...
void CLightInfo::Defaults( void ) {
	...
	lightRadius[0] = lightRadius[1] = lightRadius[2] = 512;	// sikk - was 300

	isAmbient = false;	// sikk - Ambient Light flag

// ---> sikk - Ambient Light Color
	ambColor_PosX[0] = ambColor_PosX[1] = ambColor_PosX[2] = 191;
	ambColor_NegX[0] = ambColor_NegX[1] = ambColor_NegX[2] = 191;
	ambColor_PosY[0] = ambColor_PosY[1] = ambColor_PosY[2] = 191;
	ambColor_NegY[0] = ambColor_NegY[1] = ambColor_NegY[2] = 191;
	ambColor_PosZ[0] = ambColor_PosZ[1] = ambColor_PosZ[2] = 255;
	ambColor_NegZ[0] = ambColor_NegZ[1] = ambColor_NegZ[2] = 127;
// <--- sikk - Ambient Light Color

	ambRotation.Zero();	// sikk - Ambient Light Rotation
	intensity = 1.0f;	// sikk - Light Intensity
}

void CLightInfo::DefaultPoint( void ) {
...
void CLightInfo::DefaultProjected( void ) {
	idVec3 oldColor = color;
	Defaults();
	color = oldColor;
	pointLight = false;
...
void CLightInfo::FromDict( const idDict *e ) {
	...
	isAmbient = e->GetBool( "ambient" );	// sikk - Ambient Light flag
...
// ---> sikk - Ambient Light Rotation
		if ( !e->GetVector( "ambRotation", "", ambRotation ) ) {
			ambRotation[0] = ambRotation[1] = ambRotation[2] = 0.0;
		}
// <--- sikk - Ambient Light Rotation
	}

	e->GetFloat( "intensity", "1.0", intensity );	// sikk - Light Intensity
}
...
void CLightInfo::ToDictWriteAllInfo( idDict *e ) {
    ...
    e->Set( "ambient", isAmbient ? "1" : "0" );	// sikk - Ambient Light flag

// ---> sikk - Ambient Light Color
	temp = ambColor_PosX;
	temp /= 255;
	e->SetVector( "ambcolor_x_pos", temp );
	temp = ambColor_NegX;
	temp /= 255;
	e->SetVector( "ambcolor_x_neg", temp );
	temp = ambColor_PosY;
	temp /= 255;
	e->SetVector( "ambcolor_y_pos", temp );
	temp = ambColor_NegY;
	temp /= 255;
	e->SetVector( "ambcolor_y_neg", temp );
	temp = ambColor_PosZ;
	temp /= 255;
	e->SetVector( "ambcolor_z_pos", temp );
	temp = ambColor_NegZ;
	temp /= 255;
	e->SetVector( "ambcolor_z_neg", temp );
// <--- sikk - Ambient Light Color

	e->SetVector( "ambRotation", ambRotation );	// sikk - Ambient Light Rotation

	e->SetFloat( "intensity", intensity );	// sikk - Light Intensity
}
...
void CLightInfo::ToDict( idDict *e ) {
	...
	e->Delete( "ambient" );	// sikk - Ambient Light flag
// ---> sikk - Ambient Light Color
	e->Delete( "ambcolor_x_pos" );
	e->Delete( "ambcolor_x_neg" );
	e->Delete( "ambcolor_y_pos" );
	e->Delete( "ambcolor_y_neg" );
	e->Delete( "ambcolor_z_pos" );
	e->Delete( "ambcolor_z_neg" );
// <--- sikk - Ambient Light Color
	e->Delete( "ambRotation" );	// sikk - Ambient Light Rotation
...
// ---> sikk - Ambient Light Rotation
			e->SetVector( "ambRotation", ambRotation );
// <--- sikk - Ambient Light Rotation
		}
// <--- sikk - Ambient Light flag
	} else {
...
// ---> sikk - Light Intensity
	e->Delete( "intensity" );
	e->SetFloat( "intensity", intensity );
// <--- sikk - Light Intensity
}
...
CLightDlg::CLightDlg( CWnd* pParent /*=NULL*/ )
    ...
    m_bIsAmbient = FALSE;	// sikk - Ambient Light flag
...
// <--- sikk - Ambient Light Rotation
	...
	m_fIntensity = 1.0f;	// sikk - Light Intensity
}

CLightDlg::~CLightDlg( void ) {
...
void CLightDlg::DoDataExchange( CDataExchange* pDX ) {
	...
	DDX_Check( pDX, IDC_CHECK_AMBIENT, m_bIsAmbient );	// sikk - Ambient Light Flag
...
// <--- sikk - Ambient Light Rotation
	...
	DDX_Text( pDX, IDC_EDIT_INTENSITY, m_fIntensity );	// sikk - Light Intensity
}
...
BEGIN_MESSAGE_MAP( CLightDlg, CDialog )
	...
	ON_BN_CLICKED( IDC_CHECK_AMBIENT, OnCheckAmbient )	// sikk - Ambient Light Flag
...
void CLightDlg::SetSpecifics( void ) {
...
// ---> sikk - Ambient Light flag
		GetDlgItem( IDC_EDIT_ROTATEX )->EnableWindow( lightInfo.isAmbient );
		GetDlgItem( IDC_EDIT_ROTATEY )->EnableWindow( lightInfo.isAmbient );
		GetDlgItem( IDC_EDIT_ROTATEZ )->EnableWindow( lightInfo.isAmbient );
// <--- sikk - Ambient Light flag
	} else {
...
void CLightDlg::EnableControls( void ) {
	...
	GetDlgItem( IDC_CHECK_PARALLEL )->EnableWindow( lightInfo.pointLight );	// sikk - Added

	GetDlgItem( IDC_CHECK_AMBIENT )->EnableWindow( lightInfo.pointLight );	// sikk - Ambient Light flag

// ---> sikk - Ambient Light Rotation
	GetDlgItem( IDC_EDIT_ROTATEX )->EnableWindow( lightInfo.pointLight );
	GetDlgItem( IDC_EDIT_ROTATEY )->EnableWindow( lightInfo.pointLight );
	GetDlgItem( IDC_EDIT_ROTATEZ )->EnableWindow( lightInfo.pointLight );
// <--- sikk - Ambient Light Rotation

	GetDlgItem( IDC_EDIT_INTENSITY )->EnableWindow( true );	// sikk - Light Intensity
...
void CLightDlg::UpdateDialogFromLightInfo( void ) {
	...
	m_bIsAmbient = lightInfo.isAmbient;	// sikk - Ambient Light Flag

// ---> sikk - Ambient Light Color
	VectorCopy( lightInfo.ambColor_PosX, ambColor_PosX );
	VectorCopy( lightInfo.ambColor_NegX, ambColor_NegX );
	VectorCopy( lightInfo.ambColor_PosY, ambColor_PosY );
	VectorCopy( lightInfo.ambColor_NegY, ambColor_NegY );
	VectorCopy( lightInfo.ambColor_PosZ, ambColor_PosZ );
	VectorCopy( lightInfo.ambColor_NegZ, ambColor_NegZ );
// <--- sikk - Ambient Light Color

// ---> sikk - Ambient Light Rotation
	m_rotateX = lightInfo.ambRotation[0];
	m_rotateY = lightInfo.ambRotation[1];
	m_rotateZ = lightInfo.ambRotation[2];
// <--- sikk - Ambient Light Rotation

    m_fIntensity = lightInfo.intensity;	// sikk - Light Intensity
...
void CLightDlg::UpdateLightInfoFromDialog( void ) {
	...
	lightInfo.isAmbient = ( m_bIsAmbient != FALSE );	// sikk - Ambient Light flag

// ---> sikk - Ambient Light Color
	VectorCopy( ambColor_PosX, lightInfo.ambColor_PosX );
	VectorCopy( ambColor_NegX, lightInfo.ambColor_NegX );
	VectorCopy( ambColor_PosY, lightInfo.ambColor_PosY );
	VectorCopy( ambColor_NegY, lightInfo.ambColor_NegY );
	VectorCopy( ambColor_PosZ, lightInfo.ambColor_PosZ );
	VectorCopy( ambColor_NegZ, lightInfo.ambColor_NegZ );
// <--- sikk - Ambient Light Color

// ---> sikk - Ambient Light Rotation
	lightInfo.ambRotation[0] = m_rotateX;
	lightInfo.ambRotation[1] = m_rotateY;
	lightInfo.ambRotation[2] = m_rotateZ;
// <--- sikk - Ambient Light Rotation

	lightInfo.intensity = m_fIntensity;	// sikk - Light Intensity
}
...
// ---> sikk - Ambient Light Color
		...
		case 6: {	// Neg Z
			...
			dc.SelectObject( pOldBmp );
			pBtn->SetBitmap( HBITMAP( ambColor_NegZ_Bitmap ) );
			break;
		}
	}
}
...
void CLightDlg::LoadLightTextures( void ) {
...
BOOL CLightDlg::OnInitDialog( void ) {
...
entity_t *SingleLightSelected( void ) {
...
void UpdateLightInspector( void ) {
...
void CLightDlg::OnBtnColor() {
	...
	if ( DoNewColor( &r, &g, &b, &ob, UpdateLightDialog ) ) {
		...
		color[1] = ob * g;
		color[2] = ob * b;
		ColorButtons( 0 );	// sikk - Added type argument
	}
}

void CLightDlg::OnCancel( void ) {
	CDialog::OnCancel();
}
...
BOOL CLightDlg::DestroyWindow( void ) {
	if ( GetSafeHwnd() ) {
		CRect rct;
		GetWindowRect( rct );
...
void CLightDlg::OnSelchangeComboTexture( void ) {
	UpdateData( TRUE );
	int sel = m_wndLights.GetCurSel();
	CString str;
	if ( sel >= 0 ) {
...
void CLightDlg::OnCheckCenter( void ) {
...
// ---> sikk - Ambient Light flag
void CLightDlg::OnCheckAmbient( void ) {
	if ( reinterpret_cast<CButton*>( GetDlgItem( IDC_CHECK_AMBIENT ) )->GetCheck() ) {
		lightInfo.isAmbient = true;
		lightInfo.isParallel = false;
		lightInfo.castShadows = false;
		lightInfo.hasCenter = true;
		lightInfo.lightCenter.x = 0;
		lightInfo.lightCenter.y = 0;
		lightInfo.lightCenter.z = 32;	// if you don't offset it, you can't drag it :(
	} else {
		lightInfo.isAmbient = false;
		lightInfo.hasCenter = false;
...
void CLightDlg::OnApplyDifferences( void ) {
