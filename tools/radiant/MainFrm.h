...
protected:
	...
	// Implementation
public:
	void 				UpdatePatchToolbarButtons( void );
	void 				NudgeSelection( int nDirection, float fAmount );
	void 				UpdateTextureBar( void );
	void 				SetButtonMenuStates( void );
	void 				SetTexValStatus( void );
	void 				SetGridStatus( void );
	void 				RoutineProcessing( void );
	CXYWnd* 			ActiveXY( void );
	...
	bool 				GetNurbMode( void ) { return nurbMode; }
	idCurve_NURBS<idVec2> *GetNurb( void ) { return &nurb; }
	void 				OnPrecisionCursorCycle( void );

	CXYWnd*				GetXYWnd( void ) { return m_pXYWnd; };
	CXYWnd*				GetXZWnd( void ) { return m_pXZWnd; };
	CXYWnd*				GetYZWnd( void ) { return m_pYZWnd; };
	CCamWnd*			GetCamera( void ) { return m_pCamWnd; };
	CZWnd*				GetZWnd( void ) { return m_pZWnd; };

// ---> sikk - Window Snapping
//	CInspectorDialog*	GetInspectorsWnd( void ) { return m_wndInspectors; };
	CStatusBar*			GetStatusbarWnd( void ) { return &m_wndStatusBar; };
	CToolBar*			GetToolbarWnd( void ) { return &m_wndToolBar; };
// <--- sikk - Window Snapping
...
public:
	...
	afx_msg void OnCurvePatchcube();	// sikk - Added - Cube Patch
	afx_msg void OnCurveQuickPatch();	// sikk - Added - Quick Patch
	...
	afx_msg void OnPatchInvertNormals();	// sikk - Added - Patch Invert Normals
	afx_msg void OnSnapToGrid();
	afx_msg void OnSnapAllToGrid();	// sikk - Added - snap all brushes to grid command
// ---> sikk - Added - Caulk/NoDraw/VisPortal Selected
	afx_msg void OnCaulkSelected();
	afx_msg void OnNoDrawSelected();
	afx_msg void OnVisPortalSelected();
// <--- sikk - Added - Caulk/NoDraw/VisPortal Selected
	...
	//afx_msg void OnPatchInspector();	// sikk - Merged Patch Inspector into Surface Inspector
