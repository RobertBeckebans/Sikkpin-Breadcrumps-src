...
void CreateRightClickEntity( CXYWnd *pWnd, int x, int y, char *pName ) {
	...
	brush_t *pBrush;
	if ( selected_brushes.next == &selected_brushes ) {
// ---> sikk - Specific Cam drop handling 
		if ( g_pParentWnd->GetFocus() == g_pParentWnd->GetCamera() ) {
			pBrush = g_pParentWnd->GetCamera()->CreateDropBrush();
		} else {
// <--- sikk - Specific Cam drop handling 
...
// ---> sikk - Removed Right Click Option
...
// <--- sikk - Removed Right Click Option
	...
	if ( !m_mnuDrop.GetSafeHmenu() ) {		// first time, load it up
		...
		//const int entitiesOnSubMenu = true;
...
bool CXYWnd::XY_MouseMoved(int x, int y, int buttons) {
	...
	if ( buttons == MK_LBUTTON && !m_bPress_selection && g_qeglobals.d_select_mode == sel_brush && !( m_nButtonstate & MK_RBUTTON ) ) {	// sikk - Added RMBUTTON Check
		NewBrushDrag( x, y );
		return false;
	}

	// lbutton (possibly with control and or shift) with selection = drag selection
	if ( buttons & MK_LBUTTON && !( buttons & MK_RBUTTON ) ) {	// sikk - Added RMBUTTON Check
	...
	// control mbutton = move camera
	if ( buttons == ( MK_CONTROL | nMouseButton ) ) {
		SnapToPoint( x, y, point );
		VectorCopyXY( point, g_pParentWnd->GetCamera()->Camera().origin );
	...
	// shift mbutton = move z checker
	if ( buttons == ( MK_SHIFT | nMouseButton ) ) {
		if ( RotateMode() || g_bPatchBendMode ) {
			SnapToPoint( x, y, point );
			VectorCopyXY( point, g_vRotateOrigin );
			if ( g_bPatchBendMode ) {
	...
	if ( buttons == MK_RBUTTON && !( GetAsyncKeyState( VK_MENU ) & 0x8000 ) ) {
	...
	// rbutton + lbutton = zoom xy view
	if ( buttons == ( MK_RBUTTON | MK_LBUTTON ) || ( buttons == MK_RBUTTON && ( GetAsyncKeyState( VK_MENU ) & 0x8000 ) ) ) {
		Sys_GetCursorPos( &x, &y );
		if ( y != m_ptCursor.y ) {
// sikk - Only Y axis zooms, also fixed the clamps, decreased scale, lock and hide cursor
			SetScale( Scale() + ( y - m_ptCursor.y ) * m_fScale * 0.00390625f );
			if ( Scale() > 64.0f ) {
				SetScale( 64.0f );
			}
			if ( Scale() < 0.003125f ) {
...
//	int nSize = 1.0f / g_qeglobals.d_gridsize * 256;
	...
	int stepSize = g_qeglobals.d_gridsize * 8 * fScale / 2 / m_fScale;
	if ( stepSize < g_qeglobals.d_gridsize * 8 ) {
		stepSize = g_qeglobals.d_gridsize * 8;
		//g_qeglobals.d_gridsize = g_qeglobals.d_selectedgridsize;
	} else {
		int i;
		for ( i = 1; i < stepSize; i <<= 1 ) {}
		stepSize = i;
//		if ( this->Active() ) {
			//g_qeglobals.d_gridsize = stepSize / 8;
//		}
	}
...
// <--- sikk - Fixed Grid
	...
	xb = stepSize * floor( xb / stepSize );

	xe = m_vOrigin[ nDim1 ] + w;
	if ( xe > region_maxs[ nDim1 ] ) {
		xe = region_maxs[ nDim1 ];
	}
	xe = stepSize * ceil( xe / stepSize );

	yb = m_vOrigin[ nDim2 ] - h;
	if ( yb < region_mins[ nDim2 ] ) {
		yb = region_mins[ nDim2 ];
	}
	yb = stepSize * floor( yb / stepSize );

	ye = m_vOrigin[ nDim2 ] + h;
	if ( ye > region_maxs[ nDim2 ] ) {
		ye = region_maxs[ nDim2 ];
	}
	ye = stepSize * ceil( ye / stepSize );

//---> sikk - Moved minor grid draw before major to fix the grid bug when gridsize is < 1
	// draw minor blocks
	if ( //m_fScale > fScale &&
...
//		 g_qeglobals.d_gridsize * m_fScale >= 4 &&
		...
		qglBegin( GL_LINES );
		for ( x = xb; x < xe; x += (float)stepSize / 8 ) {
// ---> sikk - Fixed grid
			//if ( !( (int)x & ( stepSize - 1 ) ) ) {
			//	continue;
			//}
//<--- sikk - Fixed grid
		...
		for ( y = yb; y < ye; y += (float)stepSize / 8 ) {
// ---> sikk - Fixed grid
			//if ( !( (int)y & ( stepSize - 1 ) ) ) {
			//	continue;
			//}
...
// ---> sikk - Fixed grid - So top coords don't get crushed
		float xb2, xe2;
		int xStepSize = g_qeglobals.d_gridsize * 8 * fScale / m_fScale;
		bool resize = false;
		if ( xStepSize < g_qeglobals.d_gridsize * 8 ) {
			xStepSize = g_qeglobals.d_gridsize * 8;
		} else {
			int i;
			for ( i = 1; i < xStepSize; i <<= 1 ) {}
			xStepSize = i;
			resize = true;
		}
		xb2 = m_vOrigin[nDim1] - w;
		if ( xb2 < region_mins[ nDim1 ] ) {
			xb2 = region_mins[ nDim1 ];
		}
		xb2 = xStepSize * floor( xb2 / xStepSize );

		xe2 = m_vOrigin[ nDim1 ] + w;
		if ( xe2 > region_maxs[ nDim1 ] ) {
			xe2 = region_maxs[ nDim1 ];
		}
		xe2 = xStepSize * ceil( xe2 / xStepSize );

		if ( resize ) {
			for ( x = xb2; x <= xe2; x += xStepSize ) {
...
//		qglRasterPos2f( m_vOrigin[ nDim1 ] - w + 35 / m_fScale, m_vOrigin[ nDim2 ] + h - 20 / m_fScale );
		...
		if ( m_nViewType == XY ) {
			cView = "XY";
			col1 = idVec3( 1.0, 0.0, 0.0 );
			col2 = idVec3( 0.0, 1.0, 0.0 );
		} else if ( m_nViewType == XZ ) {
			cView = "XZ";
			col1 = idVec3( 1.0, 0.0, 0.0 );
			col2 = idVec3( 0.0, 0.0, 1.0 );
		} else {
			cView = "YZ";
			col1 = idVec3( 0.0, 1.0, 0.0 );
			col2 = idVec3( 0.0, 0.0, 1.0 );
		}
//		qglCallLists( strlen( cView ), GL_UNSIGNED_BYTE, cView );

		qglRasterPos2f( m_vOrigin[nDim1] - w + 68 / m_fScale, m_vOrigin[nDim2] + h - 51 / m_fScale );
		qglCallLists( 1, GL_UNSIGNED_BYTE, &cView[0] );

		qglRasterPos2f( m_vOrigin[nDim1] - w + 44 / m_fScale, m_vOrigin[nDim2] + h - 28 / m_fScale);
		qglCallLists( 1, GL_UNSIGNED_BYTE, &cView[1] );

		qglLineWidth(2);
		qglColor3fv( col1.ToFloatPtr() );
		qglBegin( GL_LINES );
		qglVertex2f( m_vOrigin[nDim1] - w + 48 / m_fScale, m_vOrigin[nDim2] + h - 48 / m_fScale );
		qglVertex2f( m_vOrigin[nDim1] - w + 64 / m_fScale, m_vOrigin[nDim2] + h - 48 / m_fScale );
		qglEnd();

		qglColor3fv( col2.ToFloatPtr() );
		qglBegin( GL_LINES );
		qglVertex2f( m_vOrigin[nDim1] - w + 48 / m_fScale, m_vOrigin[nDim2] + h - 48 / m_fScale );
		qglVertex2f( m_vOrigin[nDim1] - w + 48 / m_fScale, m_vOrigin[nDim2] + h - 32 / m_fScale );
		qglEnd();
		qglLineWidth(1);
...
void CXYWnd::DrawZIcon(void) {
	if (m_nViewType == XY) {
		float size = 8 * ( 1.0f / m_fScale );	// sikk - Keep Z Icon size proportional to screen
		float	x = z.origin[0];
		float	y = z.origin[1];
		qglEnable(GL_BLEND);
		globalImages->BindNull();
		...
		qglBegin(GL_LINE_STRIP);
		qglVertex3f(x - size * 0.5f, y + size * 0.5f, 0);
		qglVertex3f(x + size * 0.5f, y + size * 0.5f, 0);
		qglVertex3f(x - size * 0.5f, y - size * 0.5f, 0);
		qglVertex3f(x + size * 0.5f, y - size * 0.5f, 0);
		qglEnd();
...
bool FilterBrush(brush_t *pb) {
	...
	if ( g_qeglobals.d_savedinfo.exclude & ( EXCLUDE_NODRAW | EXCLUDE_CAULK | EXCLUDE_VISPORTALS ) ) {	// sikk - Added nodraw to the mix
		//
		...
		while ( f ) {
			if ( g_qeglobals.d_savedinfo.exclude & EXCLUDE_NODRAW ) {
				if ( !strstr( f->texdef.name, "nodraw" ) ) {
					break;
				}
			} else  if ( g_qeglobals.d_savedinfo.exclude & EXCLUDE_CAULK ) {
	...
	if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_HINT) {
		if (strstr(pb->brush_faces->texdef.name, "aas")) {	// sikk - using "aas" in place of "hint" here (there are no "hint" brushes in idTech4)
			return true;
		}
	}
...
void DrawPathLines(void) {
	...
	if ((g_qeglobals.d_savedinfo.exclude & EXCLUDE_PATHS ) && selected_brushes.next == &selected_brushes ) {	// sikk - Added - always draw target paths if selected
		return;
	}

	num_entities = 0;
	for (te = entities.next; te != &entities && num_entities != MAX_MAP_ENTITIES; te = te->next) {
// ---> sikk - Added - always draw target paths if selected
		brush_t *b;
		bool selected = false;
		for ( b = selected_brushes.next; b != &selected_brushes; b = b->next ) {
			if ( b->owner == te ) {
		...
		if ( selected || !( g_qeglobals.d_savedinfo.exclude & EXCLUDE_PATHS ) ) {
// <--- sikk - Added - always draw target paths if selected
...
void CXYWnd::PositionView() {
	...
	brush_t *b = selected_brushes.next;
	if (b && b->next != b) {
		// sikk - let's center on the center
		//m_vOrigin[nDim1] = b->mins[nDim1];
		//m_vOrigin[nDim2] = b->mins[nDim2];
		m_vOrigin[nDim1] = ( b->maxs[nDim1] - b->mins[nDim1] ) * 0.5f + b->mins[nDim1];
		m_vOrigin[nDim2] = ( b->maxs[nDim2] - b->mins[nDim2] ) * 0.5f + b->mins[nDim2];
	}
	else {
