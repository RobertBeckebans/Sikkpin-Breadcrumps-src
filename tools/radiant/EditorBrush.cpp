...
================
*/
void Brush_Draw( brush_t* b, bool bSelected, bool bTranslucent )  	// sikk - Added - translucent arg
{
	face_t*		face;
	int			i, order;
	...
	if( b->owner->eclass->fixedsize )
	{
		...
		if( b->owner->eclass->nShowFlags & ECLASS_COMBATNODE && bSelected )  	// sikk - Added bSelected check
		{
...
// ---> sikk - Added -
			if( bTranslucent )
			{
				qglColor4f( face->d_color.x, face->d_color.y, face->d_color.z, ( face->d_texture->GetEditorAlpha() >= 1.0f ) ? 0.5f : face->d_texture->GetEditorAlpha() );	// sikk - Added -
			}
			else
			{
// <--- sikk - Added -
				qglColor4f( face->d_color.x, face->d_color.y, face->d_color.z, 1.0f );
			}
		}
...
void Brush_DrawXY( brush_t* b, int nViewType, bool bSelected, bool ignoreViewType )
{
	...
	if( b->pPatch )
	{
		Patch_DrawXY( b->pPatch, bSelected );	// sikk - Added bSelected arg
		if( !g_bPatchShowBounds )
		{
			return;
		}
	...
	if( b->owner->eclass->fixedsize )
	{
		...
		else if( b->owner->eclass->nShowFlags & ECLASS_COMBATNODE && bSelected )  	// sikk - Added bSelected check
		{
...
void Brush_Move( brush_t* b, const idVec3 move, bool bSnap, bool updateOrigin )
{
	...
	for( f = b->brush_faces; f; f = f->next )
	{
		...
		if( g_PrefsDlg.m_bTextureLock || b->owner->eclass->fixedsize )  	// sikk - Added - fixedsize entities' textures are locked
		{
