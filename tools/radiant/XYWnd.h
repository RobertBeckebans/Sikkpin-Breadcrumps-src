...
class CClipPoint {
public:
				CClipPoint( void ){ Reset(); };
	void		Reset( void ){ m_ptClip[0] = m_ptClip[1] = m_ptClip[2] = 0.0; m_bSet = false; m_pVec3 = NULL; };
	bool		Set( void ){ return m_bSet; };
	void		Set( bool b ) { m_bSet = b; };
	void		UpdatePointPtr( void ) { if ( m_pVec3 ) VectorCopy( m_ptClip, *m_pVec3 ); };
...
// Implementation
public:
	bool		AreaSelectOK( void );
	idVec3&		RotateOrigin( void );
	idVec3&		Rotation( void );
	void		UndoClear( void );
	bool		UndoAvailable( void );
	void 		KillPathMode( void );
	void 		Undo( void );
	void 		UndoCopy( void );
	void 		Copy( void );
	void 		Paste( void );
	...
	int 		GetAxisHoriz( void ) { return m_axisHoriz; };
	int 		GetAxisVert( void ) { return m_axisVert; };
	void 		AnalogMouseZoom( int mouseDeltaY );

	bool 		RogueClipMode( void );
	bool 		ClipMode( void );
	void 		SetClipMode( bool bMode );
	void 		RetainClipMode( bool bMode );

	bool 		RotateMode( void );
	...
	int 		GetViewType( void ) { return  m_nViewType; };
	void 		SetScale( float f ) { m_fScale = f; };
	float 		Scale( void ) { return m_fScale; };
	int 		Width( void ) { return m_nWidth; };
	int 		Height( void ) { return m_nHeight; };

	void 		UpdateViewDependencies( void );

	void 		DrawPrecisionCrosshair( void ); 
	void 		CyclePrecisionCrosshairMode( void );
