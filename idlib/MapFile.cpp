...
// ---> sikk - Snap Brushes on map load
			idWinding w;
			w.BaseForPlane( side->plane );
			for( int i = 0; i < 3; i++ )
			{
				side->planePts[i].x = floor( w[i].x + 0.5f );
				side->planePts[i].y = floor( w[i].y + 0.5f );
				side->planePts[i].z = floor( w[i].z + 0.5f );
			}
// <--- sikk - Snap Brushes on map load
		}
		else
...
============
*/
bool idMapBrush::Write( idFile* fp, int primitiveNum, const idVec3& origin, bool newFormat ) const  	// sikk - Old Brush Format - Added "bool newFormat" arg
{
	int i;
	idMapBrushSide* side;
// ---> sikk - Old Brush Format
...
// ---> sikk - Old Brush Format
		...
		{
			fp->WriteFloatString( "  ( %f %f %f ) ( %f %f %f ) ( %f %f %f ) ", side->planePts[0].x, side->planePts[0].y, side->planePts[0].z,
								  side->planePts[1].x, side->planePts[1].y, side->planePts[1].z,
								  side->planePts[2].x, side->planePts[2].y, side->planePts[2].z );
		}
// <--- sikk - Old Brush Format
...
============
*/
bool idMapEntity::Write( idFile* fp, int entityNum, bool newFormat ) const  	// sikk - Old Brush Format - added "bool newFormat" arg
{
	int i;
	...
	for( i = 0; i < GetNumPrimitives(); i++ )
	{
		...
		switch( mapPrim->GetType() )
		{
			case idMapPrimitive::TYPE_BRUSH:
				static_cast<idMapBrush*>( mapPrim )->Write( fp, i, origin, newFormat );	// sikk - Old Brush Format - added "bool newFormat" arg
				break;
...
============
*/
bool idMapFile::Write( const char* fileName, const char* ext, bool fromBasePath, bool newFormat )  	// sikk - Old Brush Format - added "bool newFormat" arg
{
	int i;
	idStr qpath;
	idFile* fp;
	...
	for( i = 0; i < entities.Num(); i++ )
	{
		entities[i]->Write( fp, i, newFormat );	// sikk - Old Brush Format - added "bool newFormat" arg
	}
