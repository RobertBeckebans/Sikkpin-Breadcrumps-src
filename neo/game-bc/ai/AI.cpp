...
void idAI::InitMuzzleFlash()
{
	...
	spawnArgs.GetString( "mtr_flashShader", "lights/muzzleflash", &shader );
	spawnArgs.GetVector( "flashColor", "0 0 0", flashColor );
	float flashRadius = spawnArgs.GetFloat( "flashRadius" );
	float flashBrightness = spawnArgs.GetFloat( "flashBrightness", "1.0" );	// sikk - alpha holds light brightness
	...
	worldMuzzleFlash.shaderParms[ SHADERPARM_ALPHA ] = flashBrightness;
//sikk	worldMuzzleFlash.shaderParms[ SHADERPARM_TIMESCALE ] = 1.0f;
...
void idAI::Think()
{
	...
	if( CheckDormant() || gameLocal.GetLocalPlayer()->doEndLevel )
	{
		return;
	}
...
// sikk - try to speed up pathfinding
		trace_t	trace;
//		gameLocal.clip.TracePoint( trace, org + idVec3( 0,0,8 ), move.moveDest + idVec3( 0,0,8 ), MASK_MONSTERSOLID, this );
		gameLocal.clip.TraceBounds( trace, org, move.moveDest, GetPhysics()->GetClipModel()->GetBounds(), MASK_MONSTERSOLID, this );
...
// ---> sikk - func_spawner
		...
		if( activator && activator->IsType( idSpawner::Type ) )
		{
			idSpawner* spawn = static_cast<idSpawner*>( activator );
			spawn->Attach( this );
			SetSpawner( spawn );
		}
// <--- sikk - func_spawner
...
void idAI::SetEnemyPosition()
{
	...
	else    // sikk
	{
		...
		else
		{
			const idVec3& org = physicsObj.GetOrigin();
			areaNum = PointReachableAreaNum( org );
// sikk -			if ( PathToGoal( path, areaNum, org, enemyAreaNum, pos ) ) {
			lastVisibleReachableEnemyPos = lastVisibleEnemyPos;//pos;
...
void idAI::UpdateEnemyPosition()
{
	...
	if( onGround )
	{
		...
		else
		{
			...
			{
				areaNum = PointReachableAreaNum( org );
// sikk -				if ( PathToGoal( path, areaNum, org, enemyAreaNum, enemyPos ) ) {
				lastReachableEnemyPos = enemyPos;
//				}
			}
		}
...
// sikk - Aim at origin
	if( spawnArgs.GetBool( "targetOrigin", "0" ) )
	{
		targetPos1 = aimAtEnt->GetPhysics()->GetOrigin() + idVec3( 0, 0, 16 );
		targetPos2 = aimAtEnt->GetPhysics()->GetOrigin() + idVec3( 0, 0, 32 );
	}
...
idProjectile* idAI::LaunchProjectile( const char* jointname, idEntity* target, bool clampToAttackCone )
{
	...
	float				attack_cone;
	bool				fixed_spread;		// sikk - projectile spread X/Y separated
	idVec2				projectile_spread;	// sikk - projectile spread X/Y separated
	idVec2				angle;
	float				diff;
	float				spin;
	idAngles			ang;
	...
	projectile_spread	= spawnArgs.GetVec2( "projectile_spread", "0 0" );	// sikk - projectile spread X/Y separated
	fixed_spread		= spawnArgs.GetBool( "fixed_spread", "0" );	// sikk - projectile spread X/Y separated
	...
	float increment = ( num_projectiles > 1 ) ? 1.0f / ( num_projectiles - 1 ) : 1.0f;

	idVec2 spreadRad = idVec2( DEG2RAD( projectile_spread.x ), DEG2RAD( projectile_spread.y ) );	// sikk - projectile spread X/Y separated
	idVec2 spreadRadHalf = spreadRad * 0.5f;	// sikk - projectile spread X/Y separated
	for( i = 0; i < num_projectiles; i++ )
	{
		// spread the projectiles out
		if( fixed_spread && num_projectiles > 1 )
		{
			angle.x = ( spreadRad.x * increment * i ) - spreadRadHalf.x;
			angle.y = ( spreadRad.y * increment * i ) - spreadRadHalf.y;
			dir = axis[ 0 ] + ( axis[ 2 ] * angle.y ) - ( axis[ 1 ] * angle.x );
		}
		else
		{
			angle.x = idMath::Sin( spreadRad.x * gameLocal.random.RandomFloat() );
			angle.y = idMath::Sin( spreadRad.y * gameLocal.random.RandomFloat() );
			spin = ( float )DEG2RAD( 360.0f ) * gameLocal.random.RandomFloat();
			dir = axis[ 0 ] + ( axis[ 2 ] * angle.y * idMath::Sin( spin ) ) - ( axis[ 1 ] * angle.x * idMath::Cos( spin ) );
		}
		dir.Normalize();
...
void idAI::SetChatSound()
{
	...
	{
		if( !spawnArgs.GetBool( "no_combat_chatter" ) )  	// sikk - Added no_combat_chatter flag
		{
