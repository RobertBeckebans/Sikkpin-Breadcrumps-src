...
void Cmd_TestLight_f( const idCmdArgs& args )
{
	int			i;
	idStr		filename;
	const char* key, *value, *name = "";	// sikk - warning C4701: potentially uninitialized local variable used
	idPlayer* 	player;
	idDict		dict;
...
typedef struct gameDebugLine_s
{
	bool	used;
	idVec3	start, end;
	int		color;
...
static void Cmd_SaveSelected_f( const idCmdArgs& args )
{
	...
	idDict dict;
	idStr mapName;
	const char* name = "";	// sikk - warning C4701: potentially uninitialized local variable used
...
// ---> sikk - Make Level Ambient maps command
/*
===================
Cmd_MakeLevelAmbientMaps_f
===================
*/
void Cmd_MakeLevelAmbientMaps_f( const idCmdArgs& args )
{
	int			e;
	idEntity*	ent;
	int			count;
	bool		hud, gun, shadow, fps, noclip;
	idPlayer*	player;
	const char* cubename;
	...
	hud = g_showHud.GetBool();
	gun = cvarSystem->GetCVarBool( "ui_showGun" );
	shadow = g_showPlayerShadow.GetBool();
	fps = cvarSystem->GetCVarBool( "com_showFPS" );

	player->noclip = true;
	g_showHud.SetBool( false );
	cvarSystem->SetCVarBool( "ui_showGun", false );
	g_showPlayerShadow.SetBool( false );
	cvarSystem->SetCVarBool( "com_showFPS", false );

	count = 0;
	gameLocal.Printf( "Creating Ambient Cube Maps...\n" );
	gameLocal.Printf( "--------------------------------------------------------------------\n" );
	for( e = 0; e < MAX_GENTITIES; e++ )
	{
		...
		if( idStr::Icmp( ent->spawnArgs.GetString( "classname" ), "target_cubemap" ) )
		{
			continue;
		}

		if( !ent->spawnArgs.GetString( "cubemap", "", &cubename ) )
		{
			cubename = ent->GetName();
		}

		idVec3 pos = ent->spawnArgs.GetVector( "origin" );

		// set view position to target_cubemap's origin
		pos.z -= pm_normalviewheight.GetFloat() - 0.25f;
//		player->Teleport( pos, ang, NULL );
		player->renderView->vieworg = pos;
//		player->UpdateVisuals;

		cmdSystem->BufferCommandText( CMD_EXEC_NOW, va( "screenshot %d %d", 2, 2 ) );

		// take envshot
		cmdSystem->BufferCommandText( CMD_EXEC_NOW, va( "envShot %s %d %d", cubename, 512, 32 ) );

		// make ambient map from shot (just spec for now)
		cmdSystem->BufferCommandText( CMD_EXEC_NOW, va( "makeAmbientMap %s %d %d", cubename, 32, 2 ) );


		count++;
	}

	gameLocal.Printf( "...%d cube maps created.\n", count );

	player->noclip = noclip;
	g_showHud.SetBool( hud );
	cvarSystem->SetCVarBool( "ui_showGun", gun );
	g_showPlayerShadow.SetBool( shadow );
	cvarSystem->SetCVarBool( "com_showFPS", fps );
}
// <--- sikk - Make Level Ambient maps command
...
// ---> sikk - Make Level Ambient maps command
	cmdSystem->AddCommand( "makeLevelAmbientMaps",	Cmd_MakeLevelAmbientMaps_f,	CMD_FL_GAME | CMD_FL_CHEAT,	"Creates ambient cube maps from all 'target_cubemap's in the current level." );
// <--- sikk - Make Level Ambient maps command
}
