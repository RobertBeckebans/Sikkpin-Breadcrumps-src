...
// ---> sikk - Display progress to hud if set to do so
		if( activator && activator->IsType( idPlayer::Type ) )
		{
			idPlayer* player = static_cast< idPlayer* >( activator );
			const char* msg;
			const char* msgSnd;
			spawnArgs.GetString( "messageProgress", "", &msg );
			spawnArgs.GetString( "messageProgressSound", "", &msgSnd );
			if( spawnArgs.GetBool( "showProgress" ) && msg[0] != '\0' )
			{
				player->Centerprint( va( msg, goal - count ), msgSnd );
			}
		}
// <--- sikk - Display progress to hud if set to do so
...
// ---> sikk - Display progress to hud if set to do so
	if( activator && activator->IsType( idPlayer::Type ) )
	{
		...
		const char* msg;
		const char* msgSnd;
		spawnArgs.GetString( "messageCompleted", "", &msg );
		spawnArgs.GetString( "messageCompletedSound", "", &msgSnd );
		if( spawnArgs.GetBool( "showProgress" ) && msg[0] != '\0' )
		{
			player->Centerprint( msg, msgSnd );
		}
	}
// <--- sikk - Display progress to hud if set to do so
...
idTrigger_Hurt::idTrigger_Hurt()
{
	...
	delay = 0.0f;
	nextTime = 0;
	playerOnly = false;	// sikk - affects player only when set
}
...
void idTrigger_Hurt::Save( idSaveGame* savefile ) const
{
	...
	savefile->WriteBool( playerOnly );	// sikk - affects player only when set
}
...
void idTrigger_Hurt::Restore( idRestoreGame* savefile )
{
	...
	savefile->ReadBool( playerOnly );	// sikk - affects player only when set
}
...
void idTrigger_Hurt::Spawn()
{
	...
	spawnArgs.GetBool( "playerOnly", "0", playerOnly );	// sikk - affects player only when set
...
void idTrigger_Hurt::Event_Touch( idEntity* other, trace_t* trace )
{
	...
	{
		// sikk - affects player only when set
		if( playerOnly && !other->IsType( idPlayer::Type ) )
		{
			nextTime = gameLocal.time + SEC2MS( delay );
			return;
		}
		// sikk - doesn't affect the player when set
		if( spawnArgs.GetBool( "playerSkip" ) && other->IsType( idPlayer::Type ) )
		{
			nextTime = gameLocal.time + SEC2MS( delay );
			return;
		}
