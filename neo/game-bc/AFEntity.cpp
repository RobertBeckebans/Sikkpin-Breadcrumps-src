...
void idAFEntity_Gibbable::Damage( idEntity* inflictor, idEntity* attacker, const idVec3& dir, const char* damageDefName, const float damageScale, const int location )
{
	...
	if( health <= spawnArgs.GetInt( "gibHealth", "-20" ) && spawnArgs.GetBool( "gib" ) )  	// sikk - gibHealth decl
	{
...
void idAFEntity_Gibbable::SpawnGibs( const idVec3& dir, const char* damageDefName )
{
	...
	idList<idEntity*> listRandom;	// sikk - Random Item Drops - use a second list to allow a separate remove time for items
...
// ---> sikk - Random Item drops
	// spawn random items
	idMoveableItem::DropRandomItems( this, "random", &listRandom );
// <--- sikk - Random Item drops

	idVec3 normDir = dir;
	normDir.NormalizeFast();
	//normDir = normDir * damageDef->GetInt( "push", "1" );
	...
	for( i = 0; i < list.Num(); i++ )
	{
		...
		else
		{
			...
			velocity += normDir;	// sikk - Adding "dir" here cause insane velocities so we add normalized dir
			list[i]->GetPhysics()->SetLinearVelocity( velocity * damageDef->GetInt( "gibPush", "100" ) );	// sikk - gib specific push
		}
		list[i]->GetRenderEntity()->noShadow = true;

		float rand = gameLocal.random.RandomFloat() * 15.0f + 5.0f;
		list[i]->GetRenderEntity()->shaderParms[ SHADERPARM_TIME_OF_DEATH ] = MS2SEC( gameLocal.time ) + rand - 3;
		list[i]->PostEventSec( &EV_Remove, rand );
	}

// ---> sikk - Random Item drops
	for( i = 0; i < listRandom.Num(); i++ )
	{
		listRandom[i]->GetPhysics()->SetContents( CONTENTS_CORPSE );
		listRandom[i]->GetPhysics()->SetClipMask( CONTENTS_SOLID );
		velocity = listRandom[i]->GetPhysics()->GetAbsBounds().GetCenter() - entityCenter;
		velocity.NormalizeFast();
		velocity += ( i & 1 ) ? normDir : -normDir;	// sikk - Adding "dir" here cause insane velocities so we add normalized dir
		listRandom[i]->GetPhysics()->SetLinearVelocity( velocity * 200.0f );	// sikk - was 75.0f
		listRandom[i]->GetRenderEntity()->noShadow = true;

		float rand = gameLocal.random.RandomFloat() * 10.0f + 20.0f;
		listRandom[i]->GetRenderEntity()->shaderParms[ SHADERPARM_TIME_OF_DEATH ] = MS2SEC( gameLocal.time ) + rand - 3;
		listRandom[i]->PostEventSec( &EV_Remove, rand );
	}
// <--- sikk - Random Item drops
}
...
//		if ( gameLocal.time > gameLocal.GetGibTime() ) {
		...
		renderEntity.shaderParms[ SHADERPARM_TIME_OF_DEATH ] = MS2SEC( gameLocal.time ) - 1.0f;	// sikk - gib burn is instant
...
================
*/
typedef struct jointTransformData_s
{
	renderEntity_t*		ent;
