...
// ---> sikk - Max Sound Channels
	int maxVoices = ( emitters.Num() < idSoundSystemLocal::s_maxHardwareVoices.GetInteger() ) ? emitters.Num() : idSoundSystemLocal::s_maxHardwareVoices.GetInteger();
...
*/
#ifdef _SIKKBC
	static const int MAX_PORTAL_TRACE_DEPTH = 32;	//sikk - increase this for bc
#else
...
// ---> sikk - Better stereo spatialization
		float scale = ovec.x * 0.25f + 0.75f;
		ears[0] = ( idSoundSystemLocal::s_dotbias2.GetFloat() + dot ) / ( 1.0f + dotBias ) * scale;
		ears[1] = ( idSoundSystemLocal::s_dotbias2.GetFloat() - dot ) / ( 1.0f + dotBias ) * scale;
// <--- sikk - Better stereo spatialization
		...
		int current44kHz, int numSpeakers, float* finalMixBuffer )
{
	int j;
	float volume;
	float dopplerSpeed = 1.0f;	// sikk - Doppler Hack!
...
// ---> sikk - Doppler Hack!
		dopplerSpeed = idMath::ClampFloat( -1.0f, 1.0f, ( sound->prevDistance - sound->distance ) * 0.1f ) * 0.25f + 1.0f;
		if( sound->prevDistance <= 0 )
		{
			dopplerSpeed = 1.0f;
		}
		sound->prevDistance = sound->distance;	// sikk - Doppler Hack!
// <--- sikk - Doppler Hack!
	}
	...
	else
	{
		...
		if( !global && !omni )  //slowmoActive && !chan->disallowSlow ) {	// sikk - Doppler hack!
		{
			...
			{
				slow.GatherChannelSamples( offset, MIXBUFFER_SAMPLES, alignedInputSamples, dopplerSpeed );	// sikk - Doppler hack!
			}
