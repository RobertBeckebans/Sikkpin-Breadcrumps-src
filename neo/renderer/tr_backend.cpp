...
=============
*/
const void	RB_CopyRender( const void* data, bool nearest = false )  	// sikk - nearest texture filter for framebuffer - added 'bool nearest' arg
{
	...
	if( cmd->image )
	{
		cmd->image->CopyFramebuffer( cmd->x, cmd->y, cmd->imageWidth, cmd->imageHeight, nearest );	// sikk - nearest texture filter for framebuffer - useOversizedBuffer is not used anymore so use it now for the nearest flag
	}
}

// ---> sikk - Capture Depth Buffer to Image
/*
=============
RB_CopyDepth

Copy part of the current depth buffer to an image
=============
*/
const void	RB_CopyDepth( const void* data )
{
	const copyRenderCommand_t*	cmd;
	...
	for( ; cmds ; cmds = ( const emptyCommand_t* )cmds->next )
	{
		switch( cmds->commandId )
		{
			...
			case RC_DRAW_VIEW:
				...
				break;
// ---> sikk - ssao test
			case RC_DRAW_SSAOVIEW:
				RB_DrawSSAOView( cmds );
				//if ( ( (const drawSurfsCommand_t *)cmds )->viewDef->viewEntitys ) {
				//	c_draw3d++;
				//} else {
				//	c_draw2d++;
...
// ---> sikk - nearest texture filter for framebuffer
			case RC_COPY_RENDER_NEAREST:
				RB_CopyRender( cmds, true );
				c_copyRenders++;
				break;
// <--- sikk - nearest texture filter for framebuffer
