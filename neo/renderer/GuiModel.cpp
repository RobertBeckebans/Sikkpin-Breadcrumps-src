...
void idGuiModel::EmitSurface( guiModelSurface_t* surf, float modelMatrix[16], float modelViewMatrix[16], bool depthHack )
{
	...
	memcpy( renderEntity.shaderParms, surf->shaderParms, sizeof( surf->shaderParms ) );	// sikk - pass shader parms to GUI model for post FX
...
// ---> sikk - pass shader parms to GUI model for post FX
		...
		{
			s.shaderParms[ i ] = surf->shaderParms[ i ];
		}
// <--- sikk - pass shader parms to GUI model for post FX
