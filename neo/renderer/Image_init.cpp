...
// ---> sikk - Default Depth Image Function
...
// sikk - removed normal component swapping
	int red = 0;	//( globalImages->image_useNormalCompression.GetInteger() == 1 ) ? 0 : 3;
	int alpha = 3;	//( red == 0 ) ? 3 : 0;
	...
	image->GenerateImage( ( byte* )data, 2, 2, TF_DEFAULT, true, TR_REPEAT, TD_HIGH_QUALITY );
}
...
idImage*	idImageManager::ImageFromFile( const char* _name, textureFilter_t filter, bool allowDownSize,
		textureRepeat_t repeat, textureDepth_t depth, cubeFiles_t cubeMap, bool doGammaCorrection )  	// sikk - Gamma Correction
{
	idStr name;
	idImage*	image;
	int hash;
...
// ---> sikk - Gamma Correction
			if( image->gammaCorrect != doGammaCorrection )
			{
				image->gammaCorrect = doGammaCorrection;
			}
// <--- sikk - Gamma Correction
	...
	image->filter = filter;
	image->gammaCorrect = doGammaCorrection;	// sikk - Gamma Correction
	...
	if( image->ShouldImageBePartialCached() )
	{
		...
		image->partialImage->gammaCorrect = doGammaCorrection;	// sikk - Gamma Correction
...
void idImageManager::Init()
{
	...
	portalRenderImage = ImageFromFunction( "_portalRender", R_RGBA8Image );	// sikk - portal render specific image
	currentDepthImage = ImageFromFunction( "_currentDepth", R_DepthImage );	// sikk - copy of the current depth buffer
	ssaoImage = ImageFromFunction( "_ssao", R_WhiteImage );	// sikk - ssao buffer
