...
//DrawEdges( island );
	...
	if( !opposite )
	{
		...
		common->Printf( "         vertex pos: %f %f %f\n", first->v[0], first->v[1], first->v[2] );	// sikk -
		return;
	}
...
static	void	RegenerateTriangles( optIsland_t* island )
{
	...
	for( optTri = island->tris ; optTri ; optTri = optTri->next )
	{
		...
		if( plane.Normal() * dmapGlobals.mapPlanes[ island->group->planeNum ].Normal() <= 0 )
		{
			...
			common->Printf( "         triangle pos: %f %f %f \n", island->tris->midpoint[0], island->tris->midpoint[1], island->tris->midpoint[2] );	// sikk -
			// discard it
			FreeTri( tri );
			continue;
		}
...
//	common->Printf( "      Area Number: %i\n", groupList->areaNum );	// sikk -
