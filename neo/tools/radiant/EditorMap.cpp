...
void Map_LoadFile( const char* filename )
{
	...
	if( ent )
	{
		...
		g_pParentWnd->GetCamera()->Camera().origin.z += 64;	// sikk - Added
		GetVectorForKey( ent, "origin", g_pParentWnd->GetXYWnd()->GetOrigin() );
		g_pParentWnd->GetCamera()->Camera().angles[YAW] = FloatForKey( ent, "angle" );
	}
	else
	{
// ---> sikk - Changed initial cam pos/angle
//		g_pParentWnd->GetCamera()->Camera().angles[YAW] = 0;
//		VectorCopy(vec3_origin, g_pParentWnd->GetCamera()->Camera().origin);
		g_pParentWnd->GetCamera()->Camera().origin[0] = 32.0f;
		g_pParentWnd->GetCamera()->Camera().origin[1] = -64.0f;
		g_pParentWnd->GetCamera()->Camera().origin[2] = 64.0f;
		g_pParentWnd->GetCamera()->Camera().angles[YAW] = 116.6f;
		g_pParentWnd->GetCamera()->Camera().angles[PITCH] = -45.0f;
// <--- sikk - Changed initial cam pos/angle
	...
	else
	{
		...
		for( face_t* f = b->brush_faces; f; f = f->next )
		{
			...
			if( f->dirty )
			{
				...
				f->planepts[1] -= origin;
				f->planepts[2] -= origin;
				plane.FromPoints( f->planepts[0], f->planepts[1], f->planepts[2], true );
				f->planepts[0] += origin;
				f->planepts[1] += origin;
				f->planepts[2] += origin;
			}
			...
			side->SetPlanePoints( f->planepts );	// sikk - Old Brush Format
			side->SetPlane( plane );
...
bool Map_SaveFile( const char* filename, bool use_region, bool autosave )
{
	...
	dlg.SetText( status );
	map.Write( mapFile, mapExt, !( autosave || localFile ), ( g_PrefsDlg.m_bNewMapFormat != FALSE ) );	// sikk - Old Brush Format - added "bool newFormat" arg
	mapModified = 0;
...
void Map_ImportBuffer( char* buf, bool renameEntities )
{
	...
	if( buf )
	{
		...
		while( 1 )
		{
			...
			{
// ---> sikk - Commented this out as I don't see the point in this. It just makes things messy.
...
//						sprintf( buf, "%1.10f", f );
						sprintf( buf, "%1.16f", f );	// sikk - 16 decimal places
						buf.StripTrailing( '0' );
						buf.StripTrailing( '.' );
