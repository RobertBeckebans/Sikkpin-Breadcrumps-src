...
// sikk -  removed so "finalBoss" can be used for other things
	//if ( finalBoss && !inflictor->IsType( idSoulCubeMissile::Type ) ) {
	//	return;
	//}

// ---> sikk - Monsters of the same class do not damage each other
	if( IsType( idAI::Type ) && attacker->IsType( idAI::Type ) )
	{
		const char*	classname;
		const char*	attackerclassname;
		spawnArgs.GetString( "classname", NULL, &classname );
		attacker->spawnArgs.GetString( "classname", NULL, &attackerclassname );
		if( !idStr::Icmp( classname, attackerclassname ) )
		{
			return;
		}
	}
// <--- sikk - Monsters of the same class do not damage each other
	...
	if( damage > 0 )
	{
		// sikk - Added damage debug for idActor's
		if( g_debugDamage.GetInteger() )
		{
			gameLocal.Printf( "entity:%i health:%i damage:%i damageScale:%f\n",
							  entityNumber, health, damage, damageScale );
		}
		...
		if( health <= 0 )
		{
			...
			renderEntity.shaderParms[ 9 ] = damageDef->GetInt( "killType", "0" );	// sikk - Custom Kill Types
			bool forceGib = damageDef->GetBool( "forceGib" );

			Killed( inflictor, attacker, damage, dir, location );
			if( ( health <= spawnArgs.GetInt( "gibHealth", "-20" ) && spawnArgs.GetBool( "gib" ) && damageDef->GetBool( "gib" ) ) || forceGib )
			{
				Gib( dir, damageDefName );
				renderEntity.shaderParms[ 9 ] = -1; // if it's gibbed, we don't do a burn away, just remove it immediately in the script
			}
		}
		else
		{
