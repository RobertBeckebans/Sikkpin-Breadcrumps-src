...
// ---> sikk - Modifier System
	// this is used for modifiers but can be specified for any item
	int removeDelay = spawnArgs.GetInt( "removeDelay" );
	if ( removeDelay > 0 ) {
		PostEventSec( &EV_Remove, removeDelay );
	}
// <--- sikk - Modifier System
...
// ---> sikk - Crumb count
	if ( spawnArgs.GetBool( "inv_crumb" ) ) {
		gameLocal.numCrumbs++;
	}
// <--- sikk - Crumb count
...
void idItem::Event_DropToFloor( void ) {
	...
	// sikk - increased trace form 64 to 4096
	gameLocal.clip.TraceBounds( trace, renderEntity.origin, renderEntity.origin - idVec3( 0, 0, 4096 ), renderEntity.bounds, MASK_SOLID | CONTENTS_CORPSE, this );
...
idMoveableItem::idMoveableItem() {
	...
	repeatSmoke = false;	// sikk - continuous smoke particles (for gibs)
}
...
// <--- sikk -
	...
	repeatSmoke = spawnArgs.GetBool( "repeatSmoke", "0" );	// sikk - continuous smoke particles (for gibs)
}
