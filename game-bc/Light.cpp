...
// ---> sikk - Ambient Light Color
	idVec3	ambColor_X_Neg;
	idVec3	ambColor_X_Pos;
	idVec3	ambColor_Y_Neg;
	idVec3	ambColor_Y_Pos;
	idVec3	ambColor_Z_Neg;
	idVec3	ambColor_Z_Pos;
// <--- sikk - Ambient Light Color

	idVec3	ambRotation;	// sikk - Ambient Light Rotation
	...
	if( !gotTarget )
	{
		...
		if( !args->GetVector( "light_radius", "512 512 512", renderLight->lightRadius ) )  	// sikk - Changed default radius to 512 - was 300
		{
			float radius;

			args->GetFloat( "light", "512", radius );	// sikk - Changed default radius to 512 - was 300
...
// ---> sikk - Light Intensity Parm
	if( !args->GetFloat( "intensity", "1", renderLight->shaderParms[ SHADERPARM_ALPHA ] ) )
	{
		args->GetFloat( "shaderParm3", "1", renderLight->shaderParms[ SHADERPARM_ALPHA ] );
	}
// <--- sikk - Light Intensity Parm
	...
	args->GetBool( "ambient", "0", renderLight->ambient );	// sikk - Ambient Light Flag

// ---> sikk - Ambient Light Color
	args->GetVector( "ambColor_X_Neg", "0.75 0.75 0.75",	renderLight->ambColor_X_Neg );
	args->GetVector( "ambColor_X_Pos", "0.75 0.75 0.75",	renderLight->ambColor_X_Pos );
	args->GetVector( "ambColor_Y_Neg", "0.75 0.75 0.75",	renderLight->ambColor_Y_Neg );
	args->GetVector( "ambColor_Y_Pos", "0.75 0.75 0.75",	renderLight->ambColor_Y_Pos );
	args->GetVector( "ambColor_Z_Neg", "0.5 0.5 0.5",		renderLight->ambColor_Z_Neg );
	args->GetVector( "ambColor_Z_Pos", "1.0 1.0 1.0",		renderLight->ambColor_Z_Pos );
// <--- sikk - Ambient Light Color

// ---> sikk - Ambient Light Color
	args->GetVector( "ambRotation", "0.0 0.0 0.0", renderLight->ambRotation );
// <--- sikk - Ambient Light Color

// ---> sikk - Default Light Materials
	if( !gotTarget )    // point light
	{
		if( renderLight->parallel )    // point light
		{
			args->GetString( "texture", "lights/defaultParallelLight", &texture );
		}
		else
		{
			args->GetString( "texture", "lights/defaultPointLight", &texture );
		}
	}
	else  			// projected light
	{
		args->GetString( "texture", "lights/defaultProjectedLight", &texture );
	}
// <--- sikk - Default Light Materials
...
// ---> sikk - Light LOD System
	savefile->WriteBool( lodState );
	savefile->WriteInt( lodDistance );
// <--- sikk - Light LOD System
}
...
// ---> sikk - Light LOD System
	savefile->ReadBool( lodState );
	savefile->ReadInt( lodDistance );
// <--- sikk - Light LOD System
...
void idLight::Spawn()
{
	...
	spawnArgs.GetInt( "lodDistance", "0", lodDistance );	// sikk - Light LOD System
...
void idLight::FadeIn( float time )
{
	idVec3 color;
	idVec4 color4;
	float alpha; // sikk - alpha is now used as intensity value

	currentLevel = levels;
	spawnArgs.GetVector( "_color", "1 1 1", color );
	spawnArgs.GetFloat( "shaderparm3", "1.0", alpha ); // sikk - alpha is now used as intensity value
	color4.Set( color.x, color.y, color.z, alpha );
	Fade( color4, time );
}
...
// ---> sikk - Light LOD System
	if( lodDistance > 0 )
	{
		float dist = ( GetPhysics()->GetOrigin() - gameLocal.GetLocalPlayer()->GetRenderView()->vieworg ).LengthFast();
		if( lodState && dist > lodDistance )
		{
			FadeOut( 1 );
			lodState = 0;
		}
		else if( !lodState && dist < lodDistance )
		{
			FadeIn( 1 );
			lodState = 1;
		}
