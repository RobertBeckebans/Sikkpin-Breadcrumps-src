...
void idMover::Spawn( void ) {
	...
	acceltime		= SEC2MS( spawnArgs.GetFloat( "accel_time", "0" ) );
	deceltime		= SEC2MS( spawnArgs.GetFloat( "decel_time", "0" ) );
	move_time		= SEC2MS( spawnArgs.GetFloat( "move_time", "1" ) );	// safe default value
...
void idMover::BeginMove( idThread *thread ) {
	...
	if ( move_speed ) {
		...
		acceldist = MS2SEC( totalacceltime * 0.5f ) * move_speed;
		if ( acceldist >= dist ) {
			...
			move_time = totalacceltime + SEC2MS( dist - acceldist ) / move_speed;
		}
	}
...
void idMover::Event_Rotate( idAngles &angles ) {
	...
	physicsObj.GetLocalAngles( ang );
	dest_angles = ang + angles * MS2SEC( move_time - ( acceltime + deceltime ) / 2 );
...
void idMover::Event_Bob( float speed, float phase, idVec3 &depth ) {
	...
	physicsObj.GetLocalOrigin( org );
	physicsObj.SetLinearExtrapolation( extrapolation_t(EXTRAPOLATION_DECELSINE|EXTRAPOLATION_NOSTOP), SEC2MS( speed ) * phase, speed * 500, org, depth * 2.0f, vec3_origin );
}
...
void idMover::Event_Sway( float speed, float phase, idAngles &depth ) {
	...
	physicsObj.SetAngularExtrapolation( extrapolation_t(EXTRAPOLATION_DECELSINE|EXTRAPOLATION_NOSTOP), SEC2MS( duration ) * phase, SEC2MS( duration ), ang, angSpeed, ang_zero );
}
...
// ---> sikk - Centerprint Messages
			if ( other && other->IsType( idPlayer::Type ) ) {
				...
				const char *msg;
				spawnArgs.GetString( "lockedmsg", "", &msg );
				if ( msg[0] != '\0' ) {
					player->Centerprint( msg, "", false );
				}
			}
// <--- sikk - Centerprint Messages
		}
...
void idBobber::Spawn( void ) {
	...
	physicsObj.SetLinearExtrapolation( extrapolation_t(EXTRAPOLATION_DECELSINE|EXTRAPOLATION_NOSTOP), SEC2MS( phase ), speed * 500, GetPhysics()->GetOrigin(), delta * 2.0f, vec3_origin );
	SetPhysics( &physicsObj );
}
...
void idPendulum::Spawn( void ) {
	...
	physicsObj.SetLinearExtrapolation( EXTRAPOLATION_NONE, 0, 0, GetPhysics()->GetOrigin(), vec3_origin, vec3_origin );
	physicsObj.SetAngularExtrapolation( extrapolation_t(EXTRAPOLATION_DECELSINE|EXTRAPOLATION_NOSTOP), SEC2MS( phase ), 500/freq, GetPhysics()->GetAxis().ToAngles(), idAngles( 0, 0, speed * 2.0f ), ang_zero );
	SetPhysics( &physicsObj );
}
...
void idRiser::Event_Activate( idEntity *activator ) {
	...
	if ( !IsHidden() && spawnArgs.GetBool("hide")  ) {
		...
		physicsObj.SetLinearExtrapolation( EXTRAPOLATION_LINEAR, gameLocal.time, SEC2MS( time ), physicsObj.GetOrigin(), delta, vec3_origin );
	}
}
...
const idEventDef EV_Button_StartOpen( "<startOpen>", NULL );
...
CLASS_DECLARATION( idMover_Binary, idButton )
	...
	EVENT( EV_Button_StartOpen,				idButton::Event_StartOpen )
...
void idButton::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteObject( companionButton );
}
...
void idButton::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadObject( reinterpret_cast<idClass *&>( companionButton ) );
}
...
void idButton::Spawn( void ) {
	...
	// expects syncLock to be a button that must be closed before this button will open
	spawnArgs.GetString( "syncLock", "", syncLock );
	...
	// ever separate piece of a button is considered solid when other team mates push entities
	fl.solidForTeam = true;
	...
	if ( start_open ) {
		// post it after EV_SpawnBind
		PostEventMS( &EV_Button_StartOpen, 1 );		
	}
	...
	if ( moveMaster == this ) {
		...
		if ( noTouch || health ) {
			// non touch/shoot buttons
			PostEventMS( &EV_Mover_MatchTeam, 0, moverState, gameLocal.time );

			const char *sndtemp = spawnArgs.GetString( "snd_locked" );
			if ( spawnArgs.GetInt( "locked" ) && sndtemp && *sndtemp ) {
				PostEventMS( &EV_Button_SpawnSoundTrigger, 0 );
			}
		} else {
			// spawn trigger
...
================
*/
void idButton::Hide( void ) {
	idMover_Binary *slave;
	idMover_Binary *master;
	idButton *slaveButton;
	idButton *companion;
	...
	if ( this != master ) {
		...
		for ( slave = this; slave != NULL; slave = slave->GetActivateChain() ) {
			if ( slave->IsType( idButton::Type ) ) {
				slaveButton = static_cast<idButton *>( slave );
				companion = slaveButton->companionButton;
				if ( companion && ( companion != master ) && ( companion->GetMoveMaster() != master ) ) {
					companion->Hide();
				}
				if ( slaveButton->trigger ) {
					slaveButton->trigger->Disable();
				}
				if ( slaveButton->sndTrigger ) {
					slaveButton->sndTrigger->Disable();
				}
			}
			slave->GetPhysics()->GetClipModel()->Disable();
			slave->idMover_Binary::Hide();
	...
	if ( this != master ) {
		...
		for ( slave = this; slave != NULL; slave = slave->GetActivateChain() ) {
			if ( slave->IsType( idButton::Type ) ) {
				...
				if ( slaveButton->trigger ) {
					slaveButton->trigger->Enable();
				}
				if ( slaveButton->sndTrigger ) {
					slaveButton->sndTrigger->Enable();
				}
			}
			slave->GetPhysics()->GetClipModel()->Enable();
			slave->idMover_Binary::Show();
...
void idButton::Use( idEntity *other, idEntity *activator ) {
	if ( gameLocal.RequirementMet( activator, requires, removeItem ) ) {
		if ( syncLock.Length() ) {
			idEntity *sync = gameLocal.FindEntity( syncLock );
			if ( sync && sync->IsType( idButton::Type ) ) {
				if ( static_cast<idButton *>( sync )->IsOpen() ) {
					return;
				}
			}
		}
...
/*
======================
idButton::Event_StartOpen

if "start_open", reverse position 1 and 2
======================
*/
void idButton::Event_StartOpen( void ) {
	float time;
	float speed;
...
All of the parts of a button have been spawned, so create
a trigger that encloses all of them.
======================
*/
void idButton::Event_SpawnButtonTrigger( void ) {
	idBounds		bounds;
	idMover_Binary	*other;
	bool			toggle;
	...
	// check if any of the buttons are marked as toggled
	toggle = false;
	for( other = moveMaster; other != NULL; other = other->GetActivateChain() ) {
		if ( other->IsType( idButton::Type ) && other->spawnArgs.GetBool( "toggle" ) ) {
			toggle = true;
			break;
		}
	}
...
======================
*/
void idButton::Event_SpawnSoundTrigger( void ) {
	idBounds bounds;
...
/*
================
idButton::Event_Reached_BinaryMover
================
*/
void idButton::Event_Reached_BinaryMover( void ) {
	if ( moverState == MOVER_2TO1 ) {
		SetBlocked( false );
		const idKeyValue *kv = spawnArgs.MatchPrefix( "triggerClosed" );
		while( kv ) {
...
/*
===============
idButton::SetCompanion
===============
*/
void idButton::SetCompanion( idButton *button ) {
...
/*
================
idButton::Event_Activate
================
*/
void idButton::Event_Activate( idEntity *activator ) {
	if ( spawnArgs.GetInt( "locked" ) ) {
		if ( !trigger ) {
			PostEventMS( &EV_Button_SpawnButtonTrigger, 0 );
		}
...
================
*/
void idButton::Event_Open( void ) {
	Open();
}

/*
================
idButton::Event_Close
================
*/
void idButton::Event_Close( void ) {
	Close();
}

/*
================
idButton::Event_IsOpen
================
*/
void idButton::Event_IsOpen( void ) {
	bool state;
