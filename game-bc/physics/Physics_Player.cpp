...
const float PM_SWIMSCALE		= 1.0f;		// sikk - was 0.5f;
const float PM_LADDERSPEED		= 100.0f;
const float PM_STEPSCALE		= 1.0f;

const float PM_ACCELERATE		= 20.0f;
const float PM_AIRACCELERATE	= 1.0f;
const float PM_WATERACCELERATE	= 4.0f;
const float PM_FLYACCELERATE	= 8.0f;

const float PM_FRICTION			= 6.0f;
const float PM_CROUCHFRICTION	= 1.0f;		// sikk - Crouch Friction
...
float idPhysics_Player::CmdScale( const usercmd_t &cmd ) const {
	int		max, forwardmove, rightmove, upmove;
	float	total, scale;
...
bool idPhysics_Player::SlideMove( bool gravity, bool stepUp, bool stepDown, bool push ) {
	...
	for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ ) {
		...
		if ( stepUp && ( trace.c.normal * -gravityNormal ) < MIN_WALK_NORMAL ) {	// sikk - Added cos check for smooth step jumping
...
// ---> sikk - Added for smooth step jumping
...
void idPhysics_Player::AirMove( void ) {
	...
	idPhysics_Player::Accelerate( wishdir, wishspeed, PM_AIRACCELERATE );	// sikk - Modifier System
...
void idPhysics_Player::WalkMove( void ) {
	...
	float	wishspeed, scale, accelerate, oldVel, newVel;
	...
	if ( ( groundMaterial && groundMaterial->GetSurfaceFlags() & SURF_SLICK ) || current.movementFlags & PMF_TIME_KNOCKBACK ) {
		...
		// sikk - Double accel scale when moving backwards
...
void idPhysics_Player::NoclipMove( void ) {
	float	speed, drop, friction, newspeed, stopspeed, scale, wishspeed;
	idVec3	wishdir;
...
void idPhysics_Player::LadderMove( void ) {
	...
	if ( upscale < -ladderSpeed ) {										// sikk - Ladder Speed CVAR
		current.velocity += gravityNormal * ( upscale + ladderSpeed );	// sikk - Ladder Speed CVAR
	} else if ( upscale > ladderSpeed ) {								// sikk - Ladder Speed CVAR
		current.velocity += gravityNormal * ( upscale - ladderSpeed );	// sikk - Ladder Speed CVAR
	}
...
void idPhysics_Player::CheckLadder( void ) {
	...
	if ( walking ) {
		...
		tracedist = 32.0f;	// sikk - was 48.0f
	}
...
bool idPhysics_Player::IsCrouching( void ) const {
	return 0;//( ( current.movementFlags & PMF_DUCKED ) != 0 );// sikk - No ducking
}
...
idPhysics_Player::idPhysics_Player( void ) {
	...
	walkSpeed = 0;
	crouchSpeed = 0;
	ladderSpeed = 0;	// sikk - Ladder Speed
	maxStepHeight = 0;
	maxJumpHeight = 0;
...
void idPhysics_Player::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteFloat( ladderSpeed );	// sikk - Ladder Speed
...
void idPhysics_Player::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadFloat( ladderSpeed );	// sikk - Ladder Speed
...
================
*/
void idPhysics_Player::SetSpeed( const float newWalkSpeed, const float newCrouchSpeed, const float newLadderSpeed ) {
	walkSpeed = newWalkSpeed;
	crouchSpeed = newCrouchSpeed;
	ladderSpeed = newLadderSpeed;
}
...
bool idPhysics_Player::Evaluate( int timeStepMSec, int endTimeMSec ) {
	...
	//	gameLocal.Warning( "clip model outside world bounds for entity '%s' at (%s)", self->name.c_str(), current.origin.ToString(0) );
		gameLocal.DWarning( "clip model outside world bounds for entity '%s' at (%s)", self->name.c_str(), current.origin.ToString(0) );
	}
