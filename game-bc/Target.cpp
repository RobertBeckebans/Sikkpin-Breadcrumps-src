...
//	bool demonicOnly = spawnArgs.GetBool( "effect_demonic" );	// sikk - variable initialized but not used
...
void idTarget_FadeSoundClass::Event_RestoreVolume() {
	...
	int fadeClass = spawnArgs.GetInt( "fadeClass" );
	// restore volume
	gameSoundWorld->FadeSoundClasses( fadeClass, fadeDB, fadeTime );
}
...
CLASS_DECLARATION( idTarget, idTarget_SetPostFX )
	EVENT( EV_Activate,	idTarget_SetPostFX::Event_Activate )
END_CLASS

/*
================
idTarget_SetPostFX::Event_Activate
================
*/
void idTarget_SetPostFX::Event_Activate( idEntity *activator ) {
	const char *mtr;
	idVec3 color3;
	idVec4 color;

	spawnArgs.GetVector( "_color", "1 1 1", color3 );
	color.x = color3.x;
	color.y = color3.y;
	color.z = color3.z;
	spawnArgs.GetFloat( "shaderparm3", "1", color.w );
	spawnArgs.GetString( "material", "", &mtr );
	gameLocal.GetLocalPlayer()->SetTargetPostFX( mtr, color );
}
// <--- sikk - Target PostFX
