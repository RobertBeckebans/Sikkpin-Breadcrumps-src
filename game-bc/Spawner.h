...
ID_INLINE int idSpawner::GetNumSpawnPoints( void ) const { return spawnPoints.Num(); }

ID_INLINE idEntity* idSpawner::GetSpawnPoint( int index ) { return spawnPoints[ index ].GetEntity();}

ID_INLINE int idSpawner::GetNumActive( void ) const { return currentActive.Num(); }

ID_INLINE int idSpawner::GetMaxActive( void ) const { return maxActive; }
