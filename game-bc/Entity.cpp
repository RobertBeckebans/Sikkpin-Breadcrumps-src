...
void idGameEdit::ParseSpawnArgsToRenderEntity( const idDict *args, renderEntity_t *renderEntity ) {
	...
	renderEntity->shaderParms[ 13 ]					= args->GetFloat( "shaderParm13", "0" );
	renderEntity->shaderParms[ 14 ]					= args->GetFloat( "shaderParm14", "0" );
	renderEntity->shaderParms[ 15 ]					= args->GetFloat( "shaderParm15", "0" );
	renderEntity->shaderParms[ 16 ]					= args->GetFloat( "shaderParm16", "0" );
	renderEntity->shaderParms[ 17 ]					= args->GetFloat( "shaderParm17", "0" );
	renderEntity->shaderParms[ 18 ]					= args->GetFloat( "shaderParm18", "0" );
	renderEntity->shaderParms[ 19 ]					= args->GetFloat( "shaderParm19", "0" );
	renderEntity->shaderParms[ 20 ]					= args->GetFloat( "shaderParm20", "0" );
	renderEntity->shaderParms[ 21 ]					= args->GetFloat( "shaderParm21", "0" );
	renderEntity->shaderParms[ 22 ]					= args->GetFloat( "shaderParm22", "0" );
	renderEntity->shaderParms[ 23 ]					= args->GetFloat( "shaderParm23", "0" );
	renderEntity->shaderParms[ 24 ]					= args->GetFloat( "shaderParm24", "0" );
	renderEntity->shaderParms[ 25 ]					= args->GetFloat( "shaderParm25", "0" );
	renderEntity->shaderParms[ 26 ]					= args->GetFloat( "shaderParm26", "0" );
	renderEntity->shaderParms[ 27 ]					= args->GetFloat( "shaderParm27", "0" );
	renderEntity->shaderParms[ 28 ]					= args->GetFloat( "shaderParm28", "0" );
	renderEntity->shaderParms[ 29 ]					= args->GetFloat( "shaderParm29", "0" );
	renderEntity->shaderParms[ 30 ]					= args->GetFloat( "shaderParm30", "0" );
	renderEntity->shaderParms[ 31 ]					= args->GetFloat( "shaderParm31", "0" );
...
idEntity::~idEntity( void ) {
	if ( gameLocal.GameState() != GAMESTATE_SHUTDOWN && fl.networkSync && entityNumber >= MAX_CLIENTS ) {
		idBitMsg	msg;
...
// ---> sikk - Added: Sound functions that return length of sound file
/*
================
idEntity::StartSoundLength
================
*/
int idEntity::StartSoundLength( const char *soundName, const s_channelType channel, int soundShaderFlags, bool broadcast, int *length ) {
	const idSoundShader *shader;
	const char *sound;
	...
	shader = declManager->FindSound( sound );
	return StartSoundShaderLength( shader, channel, soundShaderFlags, broadcast, length );
}

/*
================
idEntity::StartSoundShaderLength
================
*/
int idEntity::StartSoundShaderLength( const idSoundShader *shader, const s_channelType channel, int soundShaderFlags, bool broadcast, int *length ) {
	float diversity;
	int len;
...
idAnimatedEntity::AddDamageEffect
  ...
  Damage effects track the animating impact position, spitting out particles.
==============
*/
...
// ---> sikk - Handle Combat BBox specifically (so we get blood splats and wounds
	// crude but works for now
	if ( IsType( idAI::Type ) && spawnArgs.GetBool( "use_combat_bbox" ) ) {
		const char *splat, *decal, *key;
		//const char *sound, *bleed;
		//damageEffect_t	*de;
		dir = velocity;
		dir.Normalize();

		axis = renderEntity.axis;
		origin = renderEntity.bounds.GetCenter() * renderEntity.axis;
		...
		const char *materialType = gameLocal.sufaceTypeNames[ SURFTYPE_FLESH ];
