...
// <--- sikk - Secondary Fire
	...
	flashBrightness = weaponDef->dict.GetFloat( "flashBrightness", "1.0" );	// sikk - alpha holds light brightness
	...
	muzzleFlash.shaderParms[ SHADERPARM_ALPHA ]			= flashBrightness;
//sikk	muzzleFlash.shaderParms[ SHADERPARM_TIMESCALE ]		= 1.0f;
...
// ---> sikk - Railgun Beam
	if ( railbeamModelDefHandle >= 0 ) {
		float alpha;
		if ( WEAPON_SECONDARY && gameLocal.time > railbeamTime + 3000 ) {
			trace_t tr;
			...
			idVec3 dir = beanEnd - muzzleOrigin;
			...
			railbeamRenderEntity.shaderParms[ 12 ] = length / 16;
...
//			   what is being aimed at through the center of the first person view
		...
		int contents = MASK_SHOT_RENDERMODEL | CONTENTS_PROJECTILE;
		gameLocal.clip.TracePoint( tr, start, end, contents, owner );
		// if target is less than 64 units away, us the view origin
		if ( tr.fraction < 0.001953125f ) {
			projDir = tr.endpos - playerViewOrigin;
		} else {
			projDir = tr.endpos - muzzleOrigin;
		}
		float length = projDir.Normalize();
...
// <--- sikk - Railgun Beam
				...
				gameLocal.HitScan( *projDict, playerViewOrigin, playerViewAxis[ 0 ], playerViewOrigin, owner, false, dmgPower );
// <--- sikk - Hitscan
			} else {
				if ( projectileEnt ) {
					ent = projectileEnt;
...
// sikk - removed multiplayer
			...
			if ( ent->fl.takedamage ) {
				...
				float dmgScale = ( owner->PowerUpActive( MEGADAMAGE ) * 3.0f + 1.0f );
