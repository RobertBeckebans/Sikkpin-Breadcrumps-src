...
const int idEntityFx::Duration( void ) {
	...
	for( int i = 0; i < fxEffect->events.Num(); i++ ) {
		const idFXSingleAction& fxaction = fxEffect->events[i];
		int d = SEC2MS( fxaction.delay + fxaction.duration );
		if ( d > max ) {
			max = d;
		}
	}
...
void idEntityFx::ApplyFade( const idFXSingleAction& fxaction, idFXLocalAction& laction, const int time, const int actualStart ) {
	if ( fxaction.fadeInTime || fxaction.fadeOutTime ) {
		float fadePct = (float)( time - actualStart ) / SEC2MS( fxaction.fadeInTime != 0 ? fxaction.fadeInTime : fxaction.fadeOutTime );
		if (fadePct > 1.0) {
			fadePct = 1.0;
		}
...
void idEntityFx::Run( int time ) {
	...
	for( ieff = 0; ieff < fxEffect->events.Num(); ieff++ ) {
		...
		//
		if ( laction.delay ) {
			if ( laction.start + (time - laction.start) < laction.start + SEC2MS( laction.delay ) ) {
				continue;
			}
		}
		...
		//
		int actualStart = laction.delay ? laction.start + SEC2MS(  laction.delay ) : laction.start;
		float pct = (float)( time - actualStart ) / SEC2MS( fxaction.duration );
		if ( pct >= 1.0f ) {
			laction.start = -1;
				...
				if ( !useAction->shakeStarted ) {
					...
					for ( j = 0; j < gameLocal.numClients; j++ ) {
						...
						if ( player && ( player->GetPhysics()->GetOrigin() - GetPhysics()->GetOrigin() ).LengthSqr() < Square( fxaction.shakeDistance ) ) {
							if ( !fxaction.shakeIgnoreMaster || GetBindMaster() != player ) {
